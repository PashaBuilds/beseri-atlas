---
id: aktor-karahanlilar
tip: aktor
baslik: Karahanlılar
ozet: >-
  Mâverâünnehir ve Kâşgar merkezli hanedan; yönettiği dönem, Türkçenin
  İslamî bir yazı dili olarak ilk büyük eserlerini verdiği ve bu eserlerin
  bugüne ulaştığı dönemdir.
donem: "07"
tarih_baslangic: "0840"
tarih_bitis: "1212"
bolge: [orta-asya, islam-dunyasi]
eksen: [siyasi, kulturel, dini]
guven_geneli: yaygin
etiketler: [karahanlilar, kasgar, kutadgu-bilig, divanu-lugat, turkce]
ilgili:
  - donem-07
  - aktor-samaniler
  - aktor-gokturk-kaganligi
  - kavram-bozkir-imparatorlugu
  - dusunur-biruni
  - kavram-kaynak-elestirisi
  - dusunur-nizamulmulk
  - aktor-buyuk-selcuklu
  - aktor-kanem-bornu
  - kaynak-yusuf-has-hacib-kutadgu-bilig
  - aktor-karluklar
okuma_onerisi: []
kaynaklar:
  - anahtar: k1
    tur: ansiklopedi
    ad: "TDV Islam Ansiklopedisi - KARAHANLILAR"
    url: https://islamansiklopedisi.org.tr/karahanlilar
    erisim_tarihi: 2026-08-25
    dogrulama_dizesi: "KARAHANLILAR"
    not: "Turkce akademik ansiklopedi maddesi"
  - anahtar: k2
    tur: birincil
    birincil_tur: eser
    ad: "Wisdom of Royal Glory (Kutadgu Bilig) — tam metin"
    url: https://archive.org/details/wisdom-of-royal-glory-kutadgu-bilig-a-turko-islamic-mirror-for-princes
    erisim_tarihi: 2026-08-25
    dogrulama_dizesi: "Wisdom of Royal Glory"
    not: "Yusuf Has Hacib'in eserinin Ingilizce cevirisi; katalog yazari Yusuf Hass Hajib"
  - anahtar: k3
    tur: birincil
    birincil_tur: eser
    ad: "Compendium of the Turkic Dialects, Part I — tam metin"
    url: https://archive.org/details/CompendiumOfTheTurkicDialectsPart1-MahmudAl-Kashghari
    erisim_tarihi: 2026-08-25
    dogrulama_dizesi: "Compendium of The Turkic Dialects"
    not: "Kasgarli Mahmud'un Divanu Lugati't-Turk'unun Ingilizce cevirisi, birinci cilt"
son_denetim: 2026-08-25
denetim_durumu: onaylandi
onarim_turu: 0
---

## Nerede durduğu

Karahanlılar, Mâverâünnehir ile Doğu Türkistan arasındaki bölgede hüküm süren
ve merkezlerini Balasagun ile Kâşgar'da tutan bir hanedandır.[^k1] Atlasın bölge
şemasında Orta Asya ile İslam dünyasının kesiştiği yerde durur; iki bölge
etiketi birden taşımasının sebebi budur.

Hanedanın atlas açısından ayırt edici yanı siyasi genişliği değil, yönettiği
dönemde üretilen ve bugüne ulaşan metinlerdir.[^k1][^k2][^k3] Bu dosya bu yüzden
öncelikle o metinler üzerinden kurulur.

[Sâmânîler](/aktor/samaniler/) dosyası aynı coğrafyanın bir önceki siyasi
düzenini anlatır; iki dosya birlikte okunduğunda hanedan değişiminin idari ve
kültürel süreklilikle nasıl bir arada durabildiği görünür.[^k1]

## İki metin, tek dönem

Bu dosya iki birincil metne bağlanır: bir siyaset ve ahlak kitabı ile bir
sözlük.[^k2][^k3] İkisi de aynı hanedanın yönettiği yıllarda, aynı kültürel
ortamda yazılmıştır ve ikisinin de bugün erişilebilir çevirileri vardır.

Bu, atlasın Orta Asya kapsamında ilk kez gerçekleşen bir durumdur. Bu dosyadan
önce yazılan [Sâmânîler](/aktor/samaniler/) dosyası için doğrulanabilir bir
birincil metin bulunamamış ve bu eksiklik o dosyada beyan edilmişti.[^k1]
Karahanlı döneminde durum farklıdır; fark, hanedanın daha çok kayıt tutmasından
değil, o kayıtların çevrilip dijitalleştirilmiş olmasından gelir.

Bu ayrım önemlidir. Bir dönemin "kaynak açısından zengin" görünmesi, o dönemin
daha çok yazdığını değil, yazdıklarının daha çok korunduğunu ve daha çok
çevrildiğini gösterebilir.[^k2][^k3]

## Kutadgu Bilig: yönetme üzerine bir metin

*Kutadgu Bilig*, hükümdara sunulmuş, yönetme ve erdem üzerine kurulu bir
eserdir.[^k1][^k2] Metin, soyut ilkeleri karşılıklı konuşan kişiler üzerinden
işler; her kişi bir niteliği temsil eder ve tartışma bu kişiler arasında
yürür.[^k2]

Bu biçim, eserin ne olduğunu da belirler: bir yasa derlemesi ya da tarih
kaydı değil, yöneticiye yönelik bir öğüt metnidir.[^k2] Atlas bu ayrımı
korur; bir öğüt metninden dönemin fiilî yönetim pratiği doğrudan
okunamaz.

Eserin Türkçe yazılmış olması, dönemin yazı dili tercihleri açısından ayrıca
kaydedilir.[^k1][^k2] Arapça ve Farsça yazı dili olarak kullanılmayı
sürdürürken üçüncü bir dilde bu ölçekte bir eserin verilmesi, dil ile siyasi
kimlik arasındaki bağın o dönemde nasıl kurulduğu sorusunu doğurur.

## Dîvânu Lugâti't-Türk: sözlük olarak kayıt

İkinci metin bir sözlüktür ve Türk lehçelerini Arapça konuşan bir okur için
derler.[^k1][^k3] Bir sözlüğün atlas açısından değeri, kelime listesi
olmasında değil, kelimeleri örnekleyen malzemede yatar: atasözleri, şiir
parçaları ve kullanım notları, sözlüğün derlendiği dönemin gündelik
dünyasından kesitler taşır.[^k3]

Eser ayrıca lehçeleri birbirinden ayırır ve hangi biçimin hangi toplulukta
kullanıldığını kaydeder.[^k3] Bu, bir dil topluluğunun içindeki çeşitliliğin
o dönemde fark edilip yazıya geçirildiğini gösterir.

Sözlüğün bir harita içermesi de kayda değer.[^k3] Bir dil derlemesinin
coğrafyayla birlikte sunulması, dil ile yer arasındaki bağın kurulma
biçimidir; atlasın kendi bölge şemasının sorduğu soruyla aynı ailedendir.

## İslamlaşma ve yazı dili

Hanedanın İslam'ı benimsemesi, bölgedeki siyasi ve kültürel dönüşümün
işaretlerinden biri olarak kaydedilir.[^k1] Bu benimseme, yukarıdaki iki
metnin de arka planıdır: ikisi de İslamî çerçeve içinde, ama Türkçe
yazılmıştır.[^k1][^k2][^k3]

İki unsurun bir arada durması, atlasın dinî ve kültürel eksen etiketlerini
bu dosyaya birlikte eklemesinin dayanağıdır. Din değişimini kültürel kopuş
olarak okumak, elimizdeki metinlerin varlığıyla çelişir.[^k2][^k3]

**Tartışmalı.** Karahanlıların kökeni ve hangi Türk topluluğundan geldiği tartışmalıdır;
kaynaklarda farklı görüşler birlikte aktarılır.[^k1] Atlas bu konuda bir
görüşü öne çıkarmaz. Köken sorusunun siyasi kullanımları olduğu için,
adlandırmanın kendisi de tarafsız değildir.

## Sâmânîlerden devralma

Hanedanın Mâverâünnehir'e yerleşmesi, bir önceki düzenin sona ermesiyle
birlikte gerçekleşmiştir.[^k1] Devralma, kurumların tamamen değişmesi
anlamına gelmemiştir; idari yapının önemli bölümü sürmüştür.[^k1]

Bu süreklilik, atlasın aktör dosyalarını neden hanedan düzeyinde tuttuğunun
sınırını da gösterir. Hanedan değişimi görünür bir kesittir ama tek başına
bir dönem sınırı vermez; [Dönem 07](/donem/07/) dosyası aynı yılları bütün
bölgeler üzerinden ele alır.

Sonraki yüzyıllarda hanedanın doğu ve batı kolları farklı siyasi bağımlılıklar
altına girmiştir.[^k1] Bir siyasi birimin bölünerek devam etmesi, atlasın
başlangıç ve bitiş tarihi vermesini zorlaştırır; künyede verilen tarihler
kaynaktaki genel çerçeveyi izler ve kesin sınırlar olarak okunmamalıdır.[^k1]

## Kaynak durumu

Bu dosya üç kaynağa dayanır: hanedan üzerine bir ansiklopedi maddesi[^k1] ve
dönemin iki eserinin İngilizce çevirileri.[^k2][^k3] İki birincil metnin
bulunabilmesi, bu dosyayı atlasın Orta Asya kümesindeki diğer dosyalardan
ayırır.

İki birincil kaynağın da aynı alan adında bulunması bir zayıflıktır ve burada
beyan edilir. Ayrıca ikisi de çeviridir; özgün metne değil, bir aktarıma
bağlanılmıştır. Esere yapılacak atıflarda hangi çevirinin kullanıldığı
belirtilmelidir.[^k2][^k3]

Bu dosyanın kapsamadıkları da belirtilmelidir. Hanedanın siyasi tarihi
ayrıntılı olarak işlenmez; mimari ve sanat üretimi hiç ele alınmaz. İkisi de
ayrı dosyalar gerektirir ve bu dosya yazıldığında atlasta bulunmuyorlardı.[^k1]

## İki metnin birbirini tamamlaması

İki eser aynı yıllarda yazılmıştır ama farklı okurlara seslenir.[^k2][^k3]
Biri hükümdara ve saray çevresine, öteki Arapça bilen bir okur kitlesine
yönelir. Bu ikisinin bir arada bulunması, dönemin kültürel ortamının tek
katmanlı olmadığını gösterir.

Bir öğüt metni, yönetimin nasıl olması gerektiğini anlatır; bir sözlük,
konuşulanın ne olduğunu kaydeder.[^k2][^k3] Birincisi normatiftir, ikincisi
betimleyici. Aynı dönemden iki farklı türde metnin elimizde olması, o dönemi
tek bir kaynak türünden okumanın nasıl eksik kalacağını gösterir.

Atlasın kaynak türlerini künyede ayrı ayrı işaretlemesinin sebebi budur.
Bir iddianın öğüt metninden mi yoksa kayıttan mı geldiği, iddianın ağırlığını
değiştirir; künye bu farkı gizlemez.[^k2][^k3]

Bu iki metnin varlığı, bir başka soruyu da mümkün kılar: dönemin kendisi
hakkında yazdıkları ile bugün o dönem hakkında yazılanlar arasındaki fark
nedir? Bu soru atlasın [Kaynak eleştirisi](/kavram/kaynak-elestirisi/)
dosyasında ele alınır ve burada yalnızca işaret edilir.

## Yazı dili tercihinin ağırlığı

Bir siyasi birimin hangi dilde yazdığı, kültürel etkisini ölçmenin
yollarından biridir.[^k1] Karahanlı döneminde Türkçenin bu ölçekte eser
vermesi, dilin yalnızca konuşma dili olmaktan çıkıp kayıt dili hâline
gelmesidir.[^k2][^k3]

Bu geçişin kendiliğinden olmadığını, iki metnin de kendi gerekçesini
sunmasından anlıyoruz.[^k2][^k3] Bir dilde yazmayı savunmak gerekiyorsa, o
dilde yazmak henüz olağan değil demektir; gerekçe sunma ihtiyacının kendisi
dönemin durumunu gösterir.

Atlas bu tür dolaylı işaretleri kaydetmeye önem verir. Bir metnin ne dediği
kadar, neyi açıklama gereği duyduğu da bilgi taşır.

Aynı soru başka bölgeler için de sorulabilir. Bir yazı dilinin kurumsallaşması,
[Sâmânîler](/aktor/samaniler/) dosyasında Farsça için, bu dosyada Türkçe için
kaydedilmiştir; ikisi de aynı yüzyılların içindedir ve aynı coğrafyada
gerçekleşmiştir.[^k1]

## Bu dosyanın atlastaki yeri

Bu dosya, atlasın Orta Asya kapsamını genişletme çalışmasının dördüncü
adımıdır.[^k1] Önceki üçü iki düşünür ve bir siyasi birim dosyasıydı.

Dördüncü adımda ilk kez iki birincil metin birden bağlanabildi. Bu, önceki
dosyalarda beyan edilen kaynak sıkıntısının mutlak olmadığını gösterir:
sorun bölgenin kayıt bırakmaması değil, bırakılan kayıtların ne kadarının
çevrilip erişilebilir kılındığıdır.[^k2][^k3]

Bu ayrım, atlasın kendi kapsam ölçümlerini okurken de akılda tutulmalıdır.
Bir bölgenin dosyalarında birincil kaynak azlığı, o bölge hakkında az şey
bilindiği anlamına gelmez; kullanılan arşivlerin o bölgeyi az kapsadığı
anlamına gelebilir.

## Bölünme ve bağımlılık dönemi

Hanedanın doğu ve batı kollarına ayrılması, atlasın aktör dosyalarında sık
karşılaşılan bir sorunu somutlaştırır: bir siyasi birim ikiye bölündüğünde,
ortaya çıkan iki birim aynı dosyada mı yoksa ayrı dosyalarda mı ele
alınmalıdır?[^k1]

Bu dosya ikisini birlikte tutar. Gerekçe, kaynakların da ikisini aynı
hanedanın kolları olarak ele almasıdır.[^k1] Ayrı dosyalar açmak, kaynakların
kurmadığı bir ayrımı atlasın kendisinin kurması olurdu.

Sonraki dönemde her iki kolun da dış siyasi güçlere bağımlı hâle gelmesi,
hanedanın sona erişini kademeli kılar.[^k1] Bir birimin ne zaman sona erdiğini
söylemek, o birimi neyin tanımladığına bağlıdır: hanedanın adı sürerken fiilî
gücü sona ermiş olabilir.

Aynı sorun [Sâmânîler](/aktor/samaniler/) dosyasında da kaydedilmişti;
orada bağlılık ile bağımsızlığın bir arada durabildiği belirtilmişti.[^k1]
İki dosyanın aynı yapıyı iki farklı yönden göstermesi, bunun tek bir
hanedanın istisnası değil, dönemin siyasi biçimi olduğunu düşündürür.

## Okuma yönlendirmesi

Aynı coğrafyanın bir önceki düzeni için [Sâmânîler](/aktor/samaniler/),
daha öncesi için [Göktürk Kağanlığı](/aktor/gokturk-kaganligi/) dosyaları
okunmalıdır. Bozkır siyasi düzeninin genel biçimi için
[Bozkır imparatorluğu](/kavram/bozkir-imparatorlugu/) dosyası aynı soruyu
kavram düzeyinde ele alır.

Aynı yılları bütün bölgeler üzerinden okumak için [Dönem 07](/donem/07/)
dosyası kullanılmalıdır. Dönemin ilim üretimi için
[Bîrûnî](/dusunur/biruni/) dosyası, biraz daha erken bir kuşağı gösterir.

Bu dosyanın bağlandığı iki metnin nasıl okunacağı konusunda bir uyarı
gereklidir: ikisi de çeviridir ve çeviriler kendi dönemlerinin okuma
alışkanlıklarını taşır.[^k2][^k3] Metinlere yapılacak her atıfta hangi
çevirinin kullanıldığı belirtilmelidir; özgün metinle çalışan bir okur için
bu dosya yalnızca bir başlangıç noktasıdır.
