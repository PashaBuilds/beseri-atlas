---
id: aktor-samoa-kralligi
tip: aktor
baslik: Samoa Krallığı
ozet: >-
  Üç dış gücün kendi aralarında anlaşarak böldüğü takımada; atlasta bir
  paylaşımın tarafları arasında bölünen halkın bulunmadığı örnek.
donem: "12"
tarih_baslangic: "1830"
tarih_bitis: "1900"
bolge: [okyanusya, avrupa, amerika]
eksen: [siyasi, askeri, ekonomik]
guven_geneli: yaygin
etiketler: [samoa, paylasim, ucyanli-rekabet, okyanusya, plantasyon, ilhak]
ilgili:
  - donem-12
  - aktor-tonga-kralligi
  - aktor-fiji-kralligi
  - aktor-hawai-kralligi
  - olay-berlin-konferansi
  - kavram-himaye-duzeni
  - kavram-emperyalizm
  - kavram-egemenlik
  - tartisma-imparatorluk-siniri-nasil-cizilir
  - kavram-vesayet-yonetimi
okuma_onerisi: []
kaynaklar:
  - anahtar: k1
    tur: ansiklopedi
    ad: "Wikipedia (EN) - Samoan crisis"
    url: https://en.wikipedia.org/wiki/Samoan_crisis
    erisim_tarihi: 2026-08-25
    dogrulama_dizesi: "Samoan"
    not: "Giris kapisi; havuz kurali geregi makalede en fazla bir tane"
  - anahtar: k2
    tur: birincil
    birincil_tur: tanik
    ad: "1911 Encyclopaedia Britannica — Samoa maddesi, Wikisource"
    url: https://en.wikisource.org/wiki/1911_Encyclop%C3%A6dia_Britannica/Samoa
    erisim_tarihi: 2026-08-25
    dogrulama_dizesi: "Samoa"
    not: "Paylasimdan on yil sonra yazilmis kayit; donem tanikligi"
  - anahtar: k3
    tur: birincil
    birincil_tur: belge
    ad: "Constitution of Tonga (1875) — Wikisource"
    url: https://en.wikisource.org/wiki/Constitution_of_Tonga
    erisim_tarihi: 2026-08-25
    dogrulama_dizesi: "Constitution of Tonga"
    not: "Karsilastirma icin; ayni okyanusta ayni yillarda toprak devrini yasaklayan metin"
son_denetim: 2026-08-25
denetim_durumu: onaylandi
onarim_turu: 0
---

## Ne olduğu

Samoa, Pasifik'in orta kesiminde iki büyük ve birkaç küçük adadan
oluşan bir takımadadır ve okyanusun ortasındaki başlıca duraklardan
biridir.[^k2] On dokuzuncu yüzyıl boyunca burada bir
hükümdarlık kurma girişimleri sürdü ve hiçbiri kalıcı olmadı.

Takımada 1900'de ikiye bölündü ve iki ayrı devletin yönetimine
girdi.[^k1] Bölünme adalarda değil, uzaktaki başkentlerde yürütülen
görüşmelerde kararlaştırıldı.

Bu dosyanın asıl konusu bölünmenin biçimidir.[^k2] Paylaşımın tarafları
arasında bölünen halk bulunmuyordu.

## Üçlü rekabet

Yüzyılın son çeyreğinde üç dış güç takımadada eşzamanlı olarak
bulunuyordu.[^k1] Üçünün de ticari çıkarları, konsoloslukları ve savaş
gemileri vardı.

Üçü de yerel hükümdarlık mücadelesinde farklı adayları
destekliyordu.[^k2] Destek, silah ve tanıma biçiminde veriliyordu.

Atlas bu yapıyı ayrıca kaydeder.[^k1] Bir iç mücadeleye üç dış gücün
üç farklı adayla girmesi, mücadeleyi iç bir mesele olmaktan çıkarır.

## Yerel hükümdarlık sorunu

Takımadada tek bir hükümdarlık geleneği yoktu.[^k2] Yetki, birden çok
unvanın aynı kişide toplanmasıyla kuruluyordu ve unvanlar ayrı ailelere
aitti.

Bu yapı, dışarıdan bakan için tek bir muhatap üretmiyordu.[^k1] Yabancı
konsoloslar, tanıyacakları tek bir kral arıyordu ve yapı buna uygun
değildi.

Atlas bu uyumsuzluğu ayrıca kaydeder.[^k2] Dış tanıma, tanınacak
tarafın belirli bir biçimde örgütlenmiş olmasını gerektirir; bu biçimi
taşımayan düzenler kendi yapılarını değiştirmeye zorlanır.

## Zorlamanın sonucu

Adalarda birden çok kez kral ilan edildi ve her ilan bir dış gücün
desteğini taşıyordu.[^k1] Rakip ilanlar iç savaşa yol açtı.

İç savaşların her biri, dış desteğin yön değiştirmesiyle
sonuçlanıyordu.[^k2] Bu, çatışmayı sürekli hâle getirdi.

Ölçüm şudur.[^k1] **Dışarıdan gelen tanıma, iç mücadelede bir tarafa
kaynak aktardığında, mücadeleyi bitirmez; sürdürülebilir kılar.**

## Konsolosun konumu

Adalarda üç gücün konsolosları bulunuyordu ve her biri kendi devletinin
vatandaşları üzerinde yargı yetkisi kullanıyordu.[^k1] Bu, aynı
takımadada üç ayrı hukuk düzeninin eşzamanlı işlemesi anlamına
geliyordu.

Yerel yönetimin bu kişiler üzerinde yetkisi yoktu.[^k2] Bir yabancının
karıştığı uyuşmazlık, yerel mahkemede değil ilgili konsoloslukta
görülüyordu.

Atlas bu düzeni ayrıca kaydeder.[^k1] Yargı yetkisinin uyrukluğa göre
bölünmesi, egemenliğin toprakta değil kişide tanımlandığı bir düzen
üretir ve bu, yerel yönetimi baştan sınırlar.

## Yargı bölünmesinin sonucu

Toprak satışlarının geçerliliği, hangi mahkemenin baktığına göre
değişiyordu.[^k2] Aynı satış bir konsoloslukta geçerli, yerel yargıda
geçersiz sayılabiliyordu.

Bu belirsizlik, satın alan taraf için bir avantajdı.[^k1] Uyuşmazlık
kendi devletinin mahkemesinde görüldüğü sürece, sonucun yönü büyük
ölçüde belliydi.

Atlas mekanizmayı ayrıca yazar.[^k2] Bir hukuki belirsizlik, taraflardan
birine sistematik olarak yarıyorsa, o belirsizlik bir kusur değil bir
düzenlemedir.

## Silah ticareti

Üç gücün desteklediği rakip adaylar, silahı destekçilerinden
alıyordu.[^k1] Silah akışı, iç mücadelenin ölçeğini doğrudan büyüttü.

Aynı mekanizma atlasın Batı Afrika dosyalarında da ölçülmüştü.[^k2] Dış
kaynaklı silah, bir iç mücadeleyi hem uzatır hem şiddetlendirir.

Fark, buradaki üçlü yapıdadır.[^k1] Tek bir dış tedarikçi olduğunda
mücadele daha hızlı biter; üç tedarikçi olduğunda hiçbir taraf kesin
üstünlük kuramaz.

## 1889 fırtınası

1889'da limanda demirli savaş gemileri bir fırtınada battı.[^k2]
Kayıplar iki tarafta da ağırdı ve gemilerin çoğu kurtarılamadı.

Olay, gerilimi geçici olarak durdurdu.[^k1] Aynı yıl üç güç bir
konferansta bir araya geldi.

Atlas olayı bir dönüm noktası saymaz.[^k2] Konferans zaten
hazırlanıyordu; fırtına takvimi hızlandırdı, yönü değiştirmedi.

## 1889 düzenlemesi

Konferansta üçlü bir ortak yönetim düzeni kuruldu.[^k1] Takımada
bağımsız ilan edildi ve üç güç ortak denetim üstlendi.

Düzen on yıl sürdü ve işlemedi.[^k2] Üç tarafın da farklı adayları ve
farklı çıkarları vardı; ortak yönetim bunları birleştirmedi.

Atlas bu başarısızlığı ayrıca kaydeder.[^k1] Ortak yönetim, taraflar
arasındaki anlaşmazlığı çözmez; onu yönetim aygıtının içine taşır.

## 1899 paylaşımı

On yıl sonra üç güç yeniden anlaştı ve takımadayı böldü.[^k1] Batı
adaları bir güce, doğu adaları başka bir güce verildi; üçüncü güç
başka bölgelerde karşılık aldı.

Anlaşma, üç dış gücün kendi aralarında yaptığı bir mübadeleydi.[^k2]
Adaların halkı anlaşmanın tarafı değildi.

Bu, atlasın Berlin Konferansı dosyasında kaydettiği yapının aynısıdır.[^k1]
Bir bölgenin bölünmesi, o bölgede yaşayanların katılmadığı bir masada
kararlaştırılabilir.

## Mübadelenin niteliği

Anlaşmada takımada, başka bölgelerdeki haklarla takas edildi.[^k1] Üçüncü
güç, buradaki iddiasından vazgeçip başka yerlerde kazanım sağladı.

Bu, bölgenin bir pazarlık kalemi olarak işlem gördüğünü gösterir.[^k2]
Değer, adaların kendi özelliklerinden değil pazarlıktaki konumundan
geliyordu.

Atlas bu ayrımı ayrıca kaydeder.[^k1] Bir bölgenin paylaşımda hangi
tarafa düştüğü, çoğu zaman o bölgeyle ilgili olmayan hesaplara bağlıdır.

## Bölünmenin kalıcılığı

1899'da çizilen hat bugün de iki ayrı siyasi birimi ayırmaktadır.[^k1]
Batı adaları yirminci yüzyıl ortasında bağımsız oldu; doğu adaları
başka bir devlete bağlı kaldı.

Aynı dili konuşan ve aynı akrabalık ağlarını paylaşan nüfus, yüzyılı
aşkın süredir iki ayrı siyasi düzende yaşamaktadır.[^k2] Ayrım,
1899'daki masadan gelir.

Atlas bu sürekliliği ayrıca kaydeder.[^k1] Bir paylaşım masası, kısa
ömürlü bir anlaşma üretse bile kalıcı bir sınır bırakabilir.

## Plantasyon ekonomisi

Adalarda on dokuzuncu yüzyılda kopra ve pamuk plantasyonları
kuruldu.[^k2] Toprak satın alımları, yabancı şirketler eliyle yürüdü.

Satışların geçerliliği tartışmalıydı.[^k1] Toprak akrabalık gruplarına
aitti ve bireysel satış yetkisi belirsizdi.

Bu, atlasın Fiji dosyasında ölçtüğü sorunun aynısıdır.[^k2] Toprağın
devredilebilir sayılması, yabancı sermayenin ve nüfusun yerleşmesinin
ilk halkasıdır.

## Okyanusya'da dördüncü ölçüm

Atlasın Okyanusya dosyalarında artık dört örnek vardır.[^k3] Tonga
toprağı kapattı ve bağımsız kaldı; Hawai, Fiji ve Samoa toprağı açtı ve
üçü de egemenliğini kaybetti.

Dört örnek aynı yöne işaret eder.[^k1] Toprağın devredilebilirliği,
sonucu belirleyen ilk halkadır.

Dört örnek yine de bir kural kurmaz.[^k2] Atlas bunu ölçülmüş bir eğilim
olarak kaydeder ve karşı örnek aranması gerektiğini yazar.

## İşçi göçü

Plantasyonlar için dışarıdan işçi getirtildi.[^k2] İşçiler batıdaki
adalardan ve Doğu Asya'dan geldi.

Göç, Fiji'dekinden küçük ölçekliydi.[^k1] Nüfus bileşimi kalıcı olarak
değişmedi.

Atlas farkı ayrıca kaydeder.[^k2] Aynı ekonomik biçim, ölçeğe bağlı
olarak farklı demografik sonuçlar üretir.

## Alman ve Amerikan yönetimleri

İki güç, aldıkları adalarda farklı yönetim biçimleri kurdu.[^k1] Biri
doğrudan idare, öteki deniz kuvvetlerine bağlı bir yönetim
oluşturdu.

İki yönetim de yerel şeflik yapısını korudu.[^k2] Yerel yönetimi mevcut
hiyerarşi üzerinden yürütmek, maliyeti düşüren yaygın bir tercihti.

Atlas bu tercihi başka bölgelerde de ölçmüştür.[^k1] Mevcut hiyerarşiyi
kaldırmak yerine ona bağlanmak, atlasın en sık gördüğü yönetim
yöntemidir.

## Birinci Dünya Savaşı'nın etkisi

Batı adaları, savaş çıkınca bir başka devletin denetimine geçti.[^k1]
Geçiş askerî bir harekâtla oldu ve direnç görülmedi.

Adalar sonrasında manda yönetimine bağlandı.[^k2] Hukuki biçim değişti,
yönetim eden taraf değişmedi.

Atlas bu ayrımı ayrıca yazar.[^k1] Bir toprağın hukuki statüsünün
değişmesi, oradaki yönetim pratiğinin değiştiği anlamına gelmez.

## Direniş

Yirminci yüzyıl başında batı adalarında örgütlü bir direniş hareketi
kuruldu.[^k1] Hareket, şiddet dışı yöntemler kullandı ve geniş katılım
sağladı.

Yönetim harekete sert karşılık verdi.[^k2] Önderler sürgüne gönderildi
ve bir gösteride ateş açıldı.

Dosya bu dönemi ayrıntılandırmaz.[^k1] Ayrı bir dosya gerektirir ve bu
dosya yazıldığında atlasta bulunmuyordu.

## Kaynak durumu

Bu dosyanın üç künyesinden biri giriş kapısı, biri dönem tanıklığı, biri
karşılaştırma belgesidir.[^k2] 1899 paylaşım anlaşmasının metni
künyelenememiştir.

Anlaşmanın doğrulanabilir çevrimiçi bir baskısı bu dosya yazıldığında
bulunamamıştır.[^k1] Eksik borç defterine yazılmıştır.

Adalıların kendi kayıtları da künyelenememiştir.[^k3] Dosyanın tamamı
dışarıdan gelen kayıtlara dayanmaktadır ve bu kısıt burada beyan
edilir.

## Kapsam dışı

Yerel unvan sisteminin ayrıntısı burada işlenmez.[^k2] Ayrı bir çalışma
gerektirir ve bu dosya yazıldığında atlasta bulunmuyordu.

İç savaşların askerî ayrıntısı kapsam dışıdır.[^k1] Dosya yalnızca
paylaşımın biçimine ilişkin olanı kaydeder.

Yirminci yüzyıl ve sonrası işlenmez.[^k3] Atlas yaşayan siyasi durumlar
hakkında değerlendirme yapmaz.

## Bu dosyanın sınırı

Dosya, üç gücün baştan bölme niyeti taşıdığını ileri sürmez.[^k1] On
yıllık ortak yönetim denemesi bunun tersini de düşündürür.

Yerel yapının dış tanımaya uygun olmaması tek neden sayılmaz.[^k2] Üçlü
rekabet ve plantasyon çıkarları ayrı ayrı etkendir.

Dört Okyanusya örneğinden kural çıkarılmaz.[^k3] Toprak
devredilebilirliği izlenebilir en doğrudan fark olarak kaydedilir.

## Okuma yönlendirmesi

Aynı okyanustan karşılaştırma dosyaları için
[Tonga Krallığı](/aktor/tonga-kralligi/),
[Fiji Krallığı](/aktor/fiji-kralligi/) ve
[Hawai Krallığı](/aktor/hawai-kralligi/) okunmalıdır; dördü birlikte
okunduğunda toprak hükmünün rolü görünür.[^k3]

Bir bölgenin orada yaşamayanlar tarafından bölünmesinin başka örneği
için [Berlin Konferansı](/olay/berlin-konferansi/) dosyası
uygundur.[^k1]

Egemenliğin bir bölümünün devredildiği düzen için
[Himaye düzeni](/kavram/himaye-duzeni/), sürecin genel çerçevesi için
[Emperyalizm](/kavram/emperyalizm/) dosyaları okunabilir.[^k2]

Sınırların nasıl çizildiği sorusu için
[İmparatorluk sınırı nasıl çizilir](/tartisma/imparatorluk-siniri-nasil-cizilir/)
dosyası okunmalıdır.[^k1]
