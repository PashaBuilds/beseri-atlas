---
id: dusunur-gazali
tip: dusunur
baslik: Gazâlî
ozet: >-
  Kaynak, bir konudaki görüşünün hiçbir yerde açıkça açıklanmadığını yazar;
  kendisi de son eserinde meselenin kesin olarak çözülemeyeceğini söyler.
tarih_baslangic: "1056"
tarih_bitis: "1111"
bolge: [islam-dunyasi]
eksen: [felsefi, dini, kulturel]
guven_geneli: tartismali
etiketler: [gazali, tehafut, nedensellik, ihya, ictihat]
ilgili:
  - dusunur-farabi
  - dusunur-ibn-haldun
  - tartisma-tarihsel-sayilar-nasil-okunur
  - dusunur-ibn-rusd
  - kavram-ictihad
  - kavram-tasavvuf
  - kaynak-gazali-tehafut
okuma_onerisi: []
kaynaklar:
  - anahtar: k1
    tur: akademik
    ad: "Stanford Encyclopedia of Philosophy - al-Ghazali"
    url: https://plato.stanford.edu/entries/al-ghazali/
    erisim_tarihi: 2026-08-21
    dogrulama_dizesi: "al-Ghazali"
  - anahtar: k2
    tur: ansiklopedi
    ad: "TDV Islam Ansiklopedisi - GAZZALI"
    url: https://islamansiklopedisi.org.tr/gazzali
    erisim_tarihi: 2026-08-21
    dogrulama_dizesi: "GAZZÂLÎ"
    not: "Bagimsiz alan adi; madde listeleme sayfasi"
  - anahtar: k3
    tur: akademik
    ad: "Stanford Encyclopedia of Philosophy - al-Farabi"
    url: https://plato.stanford.edu/entries/al-farabi/
    erisim_tarihi: 2026-08-21
    dogrulama_dizesi: "al-Farabi"
    not: "Elestirdigi gelenegin bir onceki halkasi"
  - anahtar: k4
    tur: birincil
    ad: "The Incoherence of the Philosophers (tam metin)"
    url: https://archive.org/details/the-incoherence-of-the-philosophers-2nd-edition-brigham-young-university-islamic
    erisim_tarihi: 2026-08-23
    dogrulama_dizesi: "The Incoherence of the Philosophers"
    not: "Dusunurun baslica eserinin kamu mali tam metni"
son_denetim: 2026-08-23
denetim_durumu: onaylandi
onarim_turu: 0
---

Kaynak onu yaklaşık 1056-1111 arasında yaşamış, Sünni İslam'ın en önde gelen ve
etkili filozof, kelamcı, fakih ve mutasavvıflarından biri olarak kaydeder.[^k1]
Bu dosya, öğretisini değil **kaynağın bir görüşü tespit edememesini** ele alır.

## Kaynak, görüşü tespit edemediğini yazar

Kaynak, onun ilgili konumunun ne yazık ki hiçbir yerde açıkça açıklanmadığını ve
ancak birbirinden kopuk ifadelerden derlenebileceğini yazar.[^k1]

::tartismali[Bir düşünürün görüşü, birincil metinlerde açıkça yazılı değil; kaynak da bunu belirtiyor. Atlas burada bir görüş atfetmez ve tespit edilememiş olmasını kaydeder.]{harita=tartisma-tarihsel-sayilar-nasil-okunur}

Bu tür kayıt boşluklarının nasıl okunacağı
[Tarihsel sayılar nasıl okunur?](/tartisma/tarihsel-sayilar-nasil-okunur/)
haritasında ele alınıyor.

## Kanıt iki yöne birden çekilir

Kaynak, *Tehâfüt*'ten sonra belirgin biçimde okazyonalist bir kozmoloji savunduğu
kitaplar yazdığını, *İhyâ*'nın otuz beşinci kitabı ile *Mişkâtü'l-envâr* gibi
başka eserlerde ise açıkça nedenselci bir dil kullandığını yazar.[^k1]

Aynı yazarın iki farklı yönde okunabilen metinler bırakmış olması,[^k1] atlasın
bir düşünüre tek bir tez atfetmemesinin nedenidir; hangi eserin okunduğuna göre
sonuç değişmektedir.

## Yazarın kendisi de karara varmaz

Kaynak, ölümünden yalnızca birkaç gün önce tamamladığı son eserinde Tanrı'nın
yaratıklarının aracılığıyla yaratıp yaratmadığını tartıştığını ve meselenin kesin
olarak çözülemeyeceğini savunduğunu yazar.[^k1]

Bir düşünürün son eserinde bir soruyu açık bırakması,[^k1] atlasın hakemlik
etmeme ilkesinin en eski örneklerinden biridir: kararsızlık da bir konumdur ve
kaydedilir.

## Eleştirdiği geleneğin bir halkası

Eleştirisinin yöneldiği felsefe geleneğinin bir önceki halkası için tutulan kayıt,
o düşünürün hayatı hakkında güvenilir pek az şey bilindiğini yazar.[^k3] O düşünür
[Fârâbî](/dusunur/farabi/) dosyasında ele alınıyor.

Bir eleştirinin, hakkında az şey bilinen bir düşünüre yöneltilmiş olması,[^k1][^k3]
tartışmanın kişilerle değil metinlerle yürüdüğünü gösterir.

## Adın yazımı da kaynağa göre değişir

Bağımsız bir ansiklopedinin madde listesi adı farklı bir imlayla tutar.[^k2]

Aynı düşünürün adının iki ansiklopedide iki ayrı biçimde yazılması,[^k1][^k2]
atlasın kimlik alanını neden ad değil kimlik dizgesi üzerinden kurduğunu gösterir.

## Bu dosyanın sınırı

Hayatı, Nizâmiye'deki görevi, *Tehâfüt*'ün içeriği, tasavvufa yönelişi ve sonraki
etkisi burada ayrı başlıklar altında ele alınmamıştır; kaynak bunları ayrı
bölümler olarak işler.[^k1]

## Eserine bağ

Bu dosya düşünürün başlıca eserinin kamu malı tam metnine bağlanır: *Tehafutu l-felasife*.[^k4] Esere yapılacak her atıf bölüm ya da paragraf numarasıyla
verilmeli; hangi baskının ve hangi çevirinin kullanıldığı belirtilmelidir.
