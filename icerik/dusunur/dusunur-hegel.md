---
id: dusunur-hegel
tip: dusunur
baslik: G. W. F. Hegel
ozet: >-
  Kaynak felsefesinin üç rakip okumasını üç ayrı alt başlıkta sıralar ve
  aralarında hakemlik etmez.
tarih_baslangic: "1770"
tarih_bitis: "1831"
bolge: [avrupa]
eksen: [felsefi, siyasi, kulturel]
guven_geneli: tartismali
etiketler: [hegel, tinin-gorungubilimi, yorum-ayrimi, schelling, jena]
ilgili:
  - dusunur-kant
  - dusunur-clausewitz
  - olay-napolyon-savaslari
  - dusunur-marx
  - kavram-sivil-toplum
  - kaynak-kant-elestiri
  - kaynak-hegel-gorungubilim
okuma_onerisi: []
kaynaklar:
  - anahtar: k1
    tur: akademik
    ad: "Stanford Encyclopedia of Philosophy - Georg Wilhelm Friedrich Hegel"
    url: https://plato.stanford.edu/entries/hegel/
    erisim_tarihi: 2026-08-21
    dogrulama_dizesi: "Hegel"
  - anahtar: k2
    tur: akademik
    ad: "Stanford Encyclopedia of Philosophy - Immanuel Kant"
    url: https://plato.stanford.edu/entries/kant/
    erisim_tarihi: 2026-08-21
    dogrulama_dizesi: "Kant"
    not: "Okumalardan birinin adini aldigi onceki dusunur"
  - anahtar: k3
    tur: ansiklopedi
    ad: "Wikipedia (EN) - Napoleonic Wars"
    url: https://en.wikipedia.org/wiki/Napoleonic_Wars
    erisim_tarihi: 2026-08-21
    dogrulama_dizesi: "Napoleonic Wars"
    not: "Universitenin isgal edildigi savaslar; bagimsiz alan adi"
  - anahtar: k4
    tur: birincil
    birincil_tur: eser
    ad: "The phenomenology of mind (tam metin)"
    url: https://archive.org/details/phenomenologyofm0000hege
    erisim_tarihi: 2026-08-23
    dogrulama_dizesi: "The phenomenology of mind"
    not: "Dusunurun baslica eserinin kamu mali tam metni"
son_denetim: 2026-08-23
denetim_durumu: onaylandi
onarim_turu: 0
---

Kaynak onu 1770-1831 arasında yaşamış, J. G. Fichte ve en azından erken
eserlerinde F. W. J. von Schelling ile birlikte anılan bir düşünür olarak
kaydeder; 1770'te Stuttgart'ta doğduğu ve 1788-1793 arasında Tübingen'de önce
felsefe sonra ilahiyat okuduğu yazılır.[^k1] Bu inceleme, felsefesini değil
**kaynağın onu nasıl bölümlediğini** ele alır.

## Üç rakip okuma üç ayrı başlıktır

Kaynak felsefesine dair okumaları alt başlıklar hâlinde sıralar: geleneksel
metafizik görüş, kimi zaman metafizik olmayan görüş de denen Kant sonrası görüş ve
Kant sonrası görüşe karşı gözden geçirilmiş metafizik tepki.[^k1]

::tartismali[Aynı ansiklopedi maddesi, konusunun felsefesi hakkında üç ayrı okuma sıralar ve hangisinin doğru olduğunu söylemez. Korpus da söylemez; üç okumanın varlığını kaydeder.]{harita=tartisma-tarihsel-sayilar-nasil-okunur}

Bu tür çoklu okumaların nasıl taşınacağı
[Tarihsel sayılar nasıl okunur?](/tartisma/tarihsel-sayilar-nasil-okunur/)
haritasında ele alınıyor. Okumalardan birinin adını aldığı düşünür
[Immanuel Kant](/dusunur/kant/) dosyasında ele alınıyor.[^k2]

## Bir ün, uzun süre başkasının gölgesinde kaydedilir

Kaynak, Schelling ile yakın çalıştıklarını, birlikte bir dergiyi düzenleyip ona
katkıda bulunduklarını ve bu ilişki nedeniyle Hegel'in uzun yıllar Schelling'in
sıradan bir takipçisi olduğu ününden kurtulamadığını yazar; Schelling'in kendisinden
beş yaş küçük olduğunu da kaydeder.[^k1]

Bir düşünürün ününün, kendi metinlerinden değil bir ilişkiden türemiş olarak
kaydedilmesi,[^k1] korpusun etki alanını neden ayrı tuttuğunu gösterir.

## Bir dostluğun bitişi bir önsözle tarihlenir

Kaynak, 1806 sonunda ilk büyük eserini tamamladığını (1807'de yayımlandı) ve
eserin daha önceki, görünüşte daha Schellingci yaklaşımından bir ayrışma
gösterdiğini yazar; Schelling'in eserin önsözündeki iğneli bir eleştiriyi kendisine
yönelik yorumladığını ve dostluklarının aniden sona erdiğini kaydeder.[^k1]

Bir metnin önsözünün bir ilişkiyi bitirmesi,[^k1] eserlerin yalnızca içerikleriyle
değil bulundukları bağlamla da kaydedilmesi gerektiğini gösterir.

## Yazım koşulları da kayıtlıdır

Kaynak, elyazmasını tamamlarken Napolyon'un birliklerinin Jena'yı işgalinin
üniversitenin faaliyetlerini kısıtladığını ve Hegel'in oradan ayrıldığını yazar;
üniversite görevi olmadan bir süre Bamberg'de gazete editörlüğü, ardından
1808-1815 arasında müdürlük yaptığını kaydeder.[^k1]

Bir başyapıtın yazılma koşulları arasında bir askerî işgalin sayılması,[^k1]
düşünce tarihini siyasi tarihten ayrı tutmanın güçlüğünü gösterir; ilgili savaşlar
[Napolyon Savaşları](/olay/napolyon-savaslari/) dosyasında ele alınıyor.[^k3]

## Kanıtın ve kapsamın sınırı
*Tinin Görüngübilimi* ve *Mantık Bilimi*'nin içeriği, ansiklopedik sistem ve tin
felsefesi burada ayrı başlıklar altında ele alınmamıştır; kaynak bunları ayrı
bölümler olarak işler.[^k1]

## Eserine bağ

Bu inceleme düşünürün başlıca eserinin kamu malı tam metnine bağlanır: *Tinin Gorungubilimi*.[^k4] Esere yapılacak her atıf bölüm ya da paragraf numarasıyla
verilmeli; hangi baskının ve hangi çevirinin kullanıldığı belirtilmelidir.
