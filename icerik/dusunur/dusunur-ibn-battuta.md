---
id: dusunur-ibn-battuta
tip: dusunur
baslik: İbn Battûta
ozet: >-
  Tanca'dan çıkıp Batı Afrika'dan Çin'e uzanan yolculuklarını anlatan gezgin;
  bıraktığı metin, gezdiği bölgelerin bir kısmı için elimizdeki en ayrıntılı
  çağdaş kayıt olmakla birlikte güvenilirliği tartışmalıdır.
tarih_baslangic: "1304"
tarih_bitis: "1369"
bolge: [afrika, islam-dunyasi, guney-asya, dogu-asya]
eksen: [kulturel, dini, ekonomik]
guven_geneli: tartismali
etiketler: [ibn-battuta, rihle, seyahatname, mali, svahili]
ilgili:
  - dusunur-ibn-haldun
  - aktor-mali-imparatorlugu
  - kavram-kaynak-elestirisi
  - kavram-ticaret-diasporasi
  - tartisma-tarihsel-sayilar-nasil-okunur
  - olay-mansa-musa-haci
  - aktor-gana-imparatorlugu
  - aktor-kanem-bornu
  - aktor-kilva
  - kavram-hint-okyanusu-ticareti
  - kavram-svahili-kiyisi
okuma_onerisi: []
kaynaklar:
  - anahtar: k1
    tur: ansiklopedi
    ad: "TDV Islam Ansiklopedisi - IBN BATTUTA"
    url: https://islamansiklopedisi.org.tr/ibn-battuta
    erisim_tarihi: 2026-08-25
    dogrulama_dizesi: "İBN BATTÛTA"
    not: "Turkce akademik ansiklopedi maddesi"
  - anahtar: k2
    tur: birincil
    birincil_tur: tanik
    ad: "The Travels of Ibn Battuta, Volume I — tam metin"
    url: https://archive.org/details/travels-of-ibn-battuta
    erisim_tarihi: 2026-08-25
    dogrulama_dizesi: "The Travels Of Ibn Battuta"
    not: "Katalog kunyesinde yazar alani Ibn Battuta olarak dogrulandi"
  - anahtar: k3
    tur: ansiklopedi
    ad: "Wikipedia (EN) - Ibn Battuta"
    url: https://en.wikipedia.org/wiki/Ibn_Battuta
    erisim_tarihi: 2026-08-25
    dogrulama_dizesi: "Ibn Battuta"
    not: "Giris kapisi; havuz kurali geregi makalede en fazla bir tane bulunur"
son_denetim: 2026-08-25
denetim_durumu: onaylandi
onarim_turu: 0
---

## Nerede durduğu

İbn Battûta, Kuzey Afrika'dan yola çıkıp uzun yıllar süren yolculuklar yapmış
ve dönüşünde gezdiklerini bir metin hâlinde bıraktırmıştır.[^k1][^k3] Atlasın
bölge şemasında dört ayrı etiket taşır; gezdiği alan, atlasın bölge kümelerinin
çoğunu keser.

Bu, düşünür dosyaları arasında olağandışı bir durumdur. Çoğu düşünür bir ya da
iki bölge etiketi taşır; burada etiketlerin çokluğu, kişinin kendisinden çok
bıraktığı metnin kapsamından gelir.[^k2]

Atlasın onu düşünür katmanında tutmasının sebebi bir kuram üretmiş olması
değildir. Bıraktığı metin, gezdiği bölgelerin bir kısmı için elimizdeki en
ayrıntılı çağdaş kayıttır ve bu metin üzerinden okunur.[^k1][^k2]

## Metnin nasıl ortaya çıktığı

Metin, yolculukların bitiminde, gezginin anlattıklarının bir kâtip tarafından
yazıya geçirilmesiyle oluşmuştur.[^k1][^k3] Bu üretim biçimi, metnin türünü ve
güvenilirliğini birlikte belirler.

Anlatılanların yıllar sonra hatırlanarak aktarılması, ayrıntıların
bulanıklaşmasına açık bir durumdur.[^k1] Bir günlük ile bir hatırat aynı
şey değildir; ikincisi, anlatıcının sonradan kurduğu bir düzeni de taşır.

Kâtibin metne kendi katkısını yapmış olması da tartışılan konular
arasındadır.[^k1][^k3] Bir metnin iki elden çıkması, hangi cümlenin kime ait
olduğunu belirsizleştirir ve bu belirsizlik metnin kendisinden çözülemez.

## Güvenilirlik tartışması

Metnin bazı bölümlerinin, gezginin kendi gözlemi değil daha önceki
yazarlardan alınmış aktarımlar olduğu ileri sürülür.[^k1][^k3] Bu iddia,
metnin tamamını geçersiz kılmaz ama her bölümün ayrı ayrı değerlendirilmesini
gerektirir.

Bazı yolculukların gerçekten yapılıp yapılmadığı da tartışılır.[^k3] Bir
gezginin anlattığı yere gitmemiş olması, o yer hakkında verdiği bilgiyi
otomatik olarak yanlış yapmaz; yalnızca bilginin kaynağını değiştirir —
gözlem değil aktarım olur.

Atlas bu ayrımı korumak zorundadır. Metinden alınan her iddia, o bölümün
gözleme mi aktarıma mı dayandığı sorusuyla birlikte taşınmalıdır.[^k2]

**Tartışmalı.** Metnin hangi bölümlerinin doğrudan gözleme dayandığı, hangilerinin daha önceki
kaynaklardan alındığı uzun süredir tartışılır. Bazı araştırmacılar belirli
güzergâhların uydurma olduğunu savunur; bazıları ise dönemin yazım
geleneklerinde başkasından aktarmanın olağan sayıldığını hatırlatır. Atlas bu
tartışmada taraf tutmaz ve metni bu çekince ile kullanır.[^k1][^k3]

## Kaydın gezdiği alanla ilişkisi

Metnin kapsadığı alanın genişliği, tek bir kişinin gözlemiyle açıklanamayacak
kadar büyüktür ve bu, metnin nasıl kurulduğu sorusunu doğurur.[^k1][^k3] Bir
gezginin otuz yıla yayılan güzergâhı, aynı zamanda dönemin ulaşım ağlarının
haritasıdır.

Güzergâhın hangi kurumlar üzerinden ilerlediği kaydedilir: hac yolları, ticaret
merkezleri ve hukuk görevlilerinin bulunduğu şehirler.[^k1] Bu ağlara bağlı
olmayan yerleşimler metinde ya hiç geçmez ya da kısa değinilerle geçer.

Bu, kaydın bir seçicilik taşıdığı anlamına gelir. Metin, gezilen dünyanın
tamamını değil, o dünyanın kurumlarla bağlantılı kesitini gösterir;[^k2]
kırsal ve ağ dışı alanlar bu kayıtta seyrek görünür.

Atlas bu seçiciliği bir kusur olarak değil, kaydın niteliği olarak
işaretler. Her kaynak bir bakış açısından yazılır; sorun bakış açısının
varlığı değil, künyede belirtilmemesidir.

Bu ilkenin pratik sonucu şudur: metinde geçmeyen bir yerleşim hakkında
kaynağın sessiz kaldığını söylemek ile orada bir şey bulunmadığını söylemek
aynı şey değildir. Sessizlik, kaydın kapsamının sınırını gösterir;
gerçekliğin sınırını değil.[^k2] Atlas bu iki cümleyi birbirinin yerine
kullanmaz ve ikincisini hiç kurmaz.

## Batı Afrika kaydı

Metnin Batı Afrika bölümü, atlasın Afrika kapsamı açısından ayrı bir ağırlık
taşır.[^k2] O bölge hakkında dönemin içinden yazılmış kayıtlar seyrektir ve bu
metin onlardan biridir.

Kaydın seyrekliği, ona duyulan ihtiyacı artırır ve aynı anda riskini de
büyütür.[^k2] Tek bir kaynağa dayanan bir bölge anlatısı, o kaynağın
eğilimlerini de devralır; karşılaştırma imkânı olmadığında hata düzeltilemez.

Atlasın [Mali İmparatorluğu](/aktor/mali-imparatorlugu/) dosyası aynı bölgeyi
siyasi birim olarak ele alır. İki dosyanın birlikte okunması, bir bölge
hakkında bildiklerimizin ne kadarının tek bir metinden geldiğini görünür kılar.

## İbn Haldûn ile aynı yüzyıl

[İbn Haldûn](/dusunur/ibn-haldun/) ile aynı yüzyılda yaşamış olmaları, iki
farklı bilgi üretme biçiminin yan yana durduğunu gösterir.[^k1] Biri
gezerek kaydeder, öteki kaydedilmiş olanı çözümler.

İki yaklaşımın aynı dönemde bulunması, atlasın düşünür katmanını neden tek bir
ölçütle kurmadığının gerekçesidir. Bir metin bırakmak ile bir kuram kurmak
farklı katkılardır; ikisi de kayda geçer.

Bu karşılaştırma, atlasın bağ yapısının işleyişine de örnektir. İki dosya
birbirine bağlandığında, ayrı ayrı okunduklarında görünmeyen bir soru ortaya
çıkar: bir yüzyıl, hangi tür bilgiyi üretmeye yatkındır?

## Ticaret ağları ve hareketlilik

Metnin kaydettiği güzergâhlar, dönemin ticaret ve hac ağlarını izler.[^k2]
Bir kişinin bu kadar geniş bir alanda hareket edebilmesi, o alanın ortak
kurumlarla bağlandığını gösterir: ortak hukuk dili, konaklama düzeni ve
tanınma mekanizmaları.

Atlasın [Ticaret diasporası](/kavram/ticaret-diasporasi/) dosyası bu ağların
kavramsal karşılığını verir. Bu dosya, kavramın tek bir kişinin güzergâhında
nasıl göründüğünü gösterir.

Hareketliliğin kendisi de bir kayıt sorunudur. Gezginin gittiği yerlerde
karşılandığı ve görevlendirildiği anlatılır;[^k1] bu, metnin anlattığı dünyanın
yalnızca belirli bir kesitini — kurumlarla bağlantılı olanı — görebildiği
anlamına gelir.

## Kaynak durumu

Bu dosya üç kaynağa dayanır: kişi üzerine bir ansiklopedi maddesi,[^k1] metnin
birinci cildinin tam metni[^k2] ve bir giriş kapısı maddesi.[^k3] Üçü üç ayrı
alan adındadır.

Künyede bir giriş kapısı kaynağının bulunması, atlasın son dönemde yazılan
makalelerinden farklıdır ve gerekçesi belirtilmelidir. Metnin diğer ciltleri
için bulunan arşiv kayıtlarının katalog künyelerinde yazar alanı boştu;
yalnızca başlık eşleşiyordu.[^k2]

Başlık eşleşmesinin tek başına yeterli sayılmaması, bu atlasta daha önce
somut bir hataya yol açtığı için benimsenmiş bir kuraldır. Bu yüzden
doğrulanmamış künyeli ciltler reddedilmiş, üçüncü kaynak olarak havuzun
makale başına bir taneye izin verdiği giriş kapısı kullanılmıştır.[^k3]

Bu tercih, kaynak sayısını korumak için kaliteden ödün vermek değildir;
tersine, doğrulanamayan bir künyeyi birincil kaynak diye göstermekten
kaçınmaktır. Hangi kaynağın ne olduğu künyede açıkça yazılıdır.

Dosyanın kapsamadıkları belirtilmelidir. Güzergâhın Hindistan, Maldivler ve
Çin bölümleri burada işlenmez; her biri ayrı dosyalar gerektirir ve bu dosya
yazıldığında atlasta bulunmuyorlardı.[^k1]

## Metnin bugüne ulaşma biçimi

Metnin elimize ulaşan hâli, doğrudan yazarın kaleminden çıkmış bir nüsha
değildir.[^k1][^k3] Yazma nüshalar arasındaki farklar ve sonraki
istinsahların katkıları, metnin tek ve sabit bir nesne olmadığını gösterir.

Bu, bu dosyanın bağlandığı çevirinin de bir seçim taşıdığı anlamına gelir.
Çevirmen belirli nüshaları esas alır ve o seçim metnin kapsamını
belirler.[^k2] Esere yapılacak atıflarda çevirinin ve cildin belirtilmesi bu
yüzden zorunludur.

Atlasın kaynak künyelerinde bu ayrımı gösterecek ayrı bir alan yoktur. Künye
bir kaydın birincil olduğunu söyler; hangi nüshadan hangi çeviriyle geldiğini
söylemez. Bu boşluk, ilgili dosyalarda nota elle yazılarak doldurulur ve
şemanın kapatmadığı bir eksikliktir.

## Neden bu dosya bu atlasta

Atlasın bu bölgeye ait kapsamı, dosya yazıldığında korpusun en dar
kümelerinden biriydi.[^k1] Bu dosya o farkı kapatmaz; kapatmaya yönelik bir
adımdır ve tek başına yeterli değildir.

Dosyanın seçilme gerekçesi yalnızca coğrafi değildir. Metin, atlasın kaynak
kuralının hem en iyi hem en zor örneğini birlikte sunar: bir bölge hakkında
dönemin içinden yazılmış ender bir kayıt, aynı zamanda güvenilirliği
tartışmalı bir metindir.[^k1][^k3]

Bu ikilik, atlasın birincil kaynak arayışının neden tek başına yeterli
olmadığını gösterir. Bir kaydın döneme ait olması onu değerli kılar;
değerli olması onu doğru kılmaz. İki nitelik ayrı ayrı değerlendirilir ve
bu dosya ikisini de künyeye yazar.

Aynı ikilik, atlasın başka bölgelerinde de karşımıza çıkacaktır. Kaynağın
seyrek olduğu her yerde, eldeki tek kaydın ağırlığı artar ve onunla birlikte
o kaydın eğilimlerinin taşınma riski de artar.[^k2]

## Okuma yönlendirmesi

Aynı yüzyılın çözümleyici tarafı için [İbn Haldûn](/dusunur/ibn-haldun/),
metnin batı bölümünün konusu için
[Mali İmparatorluğu](/aktor/mali-imparatorlugu/) dosyaları okunmalıdır.[^k1]

Metnin güvenilirlik sorununun genel biçimi için
[Kaynak eleştirisi](/kavram/kaynak-elestirisi/) dosyası, güzergâhların
dayandığı ağlar için [Ticaret diasporası](/kavram/ticaret-diasporasi/)
dosyası uygundur.

Metnin kendisine yapılacak atıflarda cilt ve bölüm numarası belirtilmeli,
o bölümün gözleme mi aktarıma mı dayandığı ayrıca not edilmelidir.[^k2]

Kaydın nasıl okunacağı konusunda son bir uyarı gereklidir. Bu dosya metnin ne
anlattığını özetlemez; metnin ne tür bir kayıt olduğunu ve hangi çekincelerle
kullanılması gerektiğini anlatır.[^k1][^k3] Anlattıklarının içeriği için
metnin kendisine inilmelidir.

Bu tercih atlasın genel tutumudur ama bu dosyada özellikle önemlidir. Bir
seyahatnamenin özeti, hangi bölümün gözleme hangisinin aktarıma dayandığı
ayrımını kaçınılmaz olarak siler; oysa bu metinde ayrımın kendisi, anlatılan
içerikten daha belirleyicidir.[^k2]

Aynı sebeple, bu dosyadaki hiçbir cümle metnin bir bölümünü doğrulanmış olgu
olarak sunmaz. Dosya, metnin varlığını ve niteliğini kaydeder; içindeki
iddiaların doğruluğu hakkında hüküm vermez.[^k1]
