---
id: dusunur-machiavelli
tip: dusunur
baslik: Machiavelli
ozet: >-
  Kaynak maddesini bir soruyla açar: neden Machiavelli? Kendisi filozof
  saymadığı gibi yazıları da kaynağa göre sistemsiz ve tutarsızdır.
tarih_baslangic: "1469"
tarih_bitis: "1527"
bolge: [avrupa]
eksen: [siyasi, felsefi, askeri]
guven_geneli: tartismali
etiketler: [machiavelli, prens, soylevler, cumhuriyetcilik, kategori]
ilgili:
  - dusunur-hobbes
  - olay-ronesans
  - tartisma-tarihsel-sayilar-nasil-okunur
  - dusunur-locke
  - dusunur-clausewitz
  - kaynak-machiavelli-prens
okuma_onerisi: []
kaynaklar:
  - anahtar: k1
    tur: akademik
    ad: "Stanford Encyclopedia of Philosophy - Niccolo Machiavelli"
    url: https://plato.stanford.edu/entries/machiavelli/
    erisim_tarihi: 2026-08-21
    dogrulama_dizesi: "Machiavelli"
  - anahtar: k2
    tur: ansiklopedi
    ad: "Wikipedia (EN) - Renaissance"
    url: https://en.wikipedia.org/wiki/Renaissance
    erisim_tarihi: 2026-08-21
    dogrulama_dizesi: "Renaissance"
    not: "Yazdigi donem; bagimsiz alan adi"
  - anahtar: k3
    tur: akademik
    ad: "Stanford Encyclopedia of Philosophy - Thomas Hobbes"
    url: https://plato.stanford.edu/entries/hobbes/
    erisim_tarihi: 2026-08-21
    dogrulama_dizesi: "Hobbes"
    not: "Sonraki siyaset dusunuru"
son_denetim: 2026-08-21
denetim_durumu: onaylandi
onarim_turu: 0
---

Kaynak maddesine bir soruyla başlar: neden Machiavelli? Bir felsefe
ansiklopedisinde onunla ilgili bir maddeye rastlayan herkesin bu soruyu doğal ve
meşru biçimde sorabileceğini yazar.[^k1] Bu dosya, öğretisini değil **kategori
sorununu** ele alır.

## Kendisini filozof saymamış görünür

Kaynak, onun kendisini filozof saymamış göründüğünü ve çoğu kez
felsefi soruşturmayı konu dışı sayarak açıkça reddettiğini yazar; nitelikleri
bakımından da standart akademik felsefe modellerine rahatça oturmadığını
kaydeder.[^k1]

::tartismali[Bir felsefe ansiklopedisi, maddesinin konusunun kendisini filozof saymadığını kendi ilk paragrafında yazıyor. Atlas bu gerilimi kategoriyi düzelterek çözmez; kaydeder.]{harita=tartisma-tarihsel-sayilar-nasil-okunur}

Bu tür kategori gerilimlerinin nasıl okunacağı
[Tarihsel sayılar nasıl okunur?](/tartisma/tarihsel-sayilar-nasil-okunur/)
haritasında ele alınıyor.

## Yazıların niteliği de kaynağın kendi ifadesidir

Kaynak yazılarını çileden çıkaracak ve herkesçe bilinir ölçüde sistemsiz ve
tutarsız olarak niteler.[^k1]

Bir külliyatın sistemsizliğinin maddenin kendisinde belirtilmesi,[^k1] atlasın bu
dosyada neden tek bir tez cümlesi kurmadığını açıklar: kaynak zaten böyle bir
cümlenin kurulamayacağını söylemektedir.

## Katkı listesi tek bir alanda değildir

Kaynak, Batı düşüncesindeki birçok önemli tartışmaya katkıda bulunduğunu yazar ve
bunları sayar: özellikle siyaset kuramı, ayrıca tarih ve tarih yazımı, İtalyan
edebiyatı, savaş ilkeleri ve diplomasi.[^k1]

Beş ayrı alanın aynı kişi için sayılması,[^k1] disiplin sınırlarının yazarın
kendisinden değil sonraki düzenlemelerden geldiğini gösterir; aynı örüntü
[Aristoteles](/dusunur/aristoteles/) dosyasında da görülür.

## Hayatı hakkında az şey bilinir

Kaynak biyografi bölümünü, hakkında görece az şey bilindiği kaydıyla açar.[^k1]

Bu, atlasın düşünür dosyalarında tekrar eden bir durumdur ve
[Fârâbî](/dusunur/farabi/) ile [Kautilya](/dusunur/kautilya/) dosyalarında da
görülür.

## Sonraki okumalar da bölümlenmiştir

Kaynak bölüm başlıkları arasında iktidarın çözümlenmesi, ahlak-din-siyaset,
devlet ve prens, Livius Üzerine Söylevler'de özgürlük ve çatışma, halk özgürlüğü
ve halk konuşması ile cumhuriyetçi liderlerin karakterini sayar; yeni bir kuşak
neo-Romen siyaset kuramcısının onun cumhuriyetçilik sürümünden ilham aldığını da
yazar.[^k1]

Aynı yazarın hem prens hem cumhuriyet başlıkları altında okunabilmesi,[^k1]
atlasın bu dosyayı neden tartışmalı olarak işaretlediğinin ikinci gerekçesidir.

## Sonraki halka

Sonraki yüzyılın siyaset düşünürü için tutulan kayıt, onun bugünkü ününün büyük
ölçüde siyaset felsefesine dayandığını yazar.[^k3] O düşünür
[Thomas Hobbes](/dusunur/hobbes/) dosyasında, yazdığı dönem ise
[Rönesans](/olay/ronesans/) dosyasında ele alınıyor.[^k2]

## Bu dosyanın sınırı

*Prens*'in içeriği, virtù ve fortuna kavramları, *Söylevler* ve Batı düşüncesindeki
yeri burada ayrı başlıklar altında ele alınmamıştır; kaynak bunları ayrı bölümler
olarak işler.[^k1]
