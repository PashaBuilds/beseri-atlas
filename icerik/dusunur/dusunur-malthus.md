---
id: dusunur-malthus
tip: dusunur
baslik: Thomas Malthus
ozet: >-
  Kaynak, eserin birinci baskısıyla sonrakiler arasındaki farkı ayrı bir
  başlıkta işler; birinci baskıda bulunan bir bölüm sonrakilerde yoktur.
tarih_baslangic: "1766"
tarih_bitis: "1834"
bolge: [avrupa, kuresel]
eksen: [demografik, ekonomik, felsefi]
guven_geneli: yaygin
etiketler: [malthus, nufus, baski-farki, ricardo, tahil-yasalari]
ilgili:
  - dusunur-smith
  - kavram-demografik-gecis
  - olay-irlanda-kitligi
  - kavram-malthus-tuzagi
  - kaynak-malthus-nufus
  - tartisma-nufus-artisi-tehdit-mi
okuma_onerisi: []
kaynaklar:
  - anahtar: k1
    tur: ansiklopedi
    ad: "Wikipedia (EN) - Thomas Robert Malthus"
    url: https://en.wikipedia.org/wiki/Thomas_Robert_Malthus
    erisim_tarihi: 2026-08-21
    dogrulama_dizesi: "Malthus"
  - anahtar: k2
    tur: akademik
    ad: "Stanford Encyclopedia of Philosophy - Adam Smith's Moral and Political Philosophy"
    url: https://plato.stanford.edu/entries/smith-moral-political/
    erisim_tarihi: 2026-08-21
    dogrulama_dizesi: "Adam Smith"
    not: "Onceki kusaktan iktisat yazari; bagimsiz alan adi"
  - anahtar: k3
    tur: veri
    ad: "Our World in Data - Population, long-run with projections (CSV verisi)"
    url: https://ourworldindata.org/grapher/population-long-run-with-projections.csv?csvType=full&useColumnShortNames=true
    erisim_tarihi: 2026-08-21
    dogrulama_dizesi: "population_historical"
    not: "Yazdigi donemin nufus serisi"
  - anahtar: k4
    tur: birincil
    birincil_tur: eser
    ad: "An Essay on the Principle of Population (tam metin)"
    url: https://www.gutenberg.org/ebooks/4239
    erisim_tarihi: 2026-08-23
    dogrulama_dizesi: "An Essay on the Principle of Population"
    not: "Dusunurun baslica eserinin kamu mali tam metni"
son_denetim: 2026-08-23
denetim_durumu: onaylandi
onarim_turu: 0
---

Kaynak *Nüfus İlkesi Üzerine Bir Deneme* için ayrı bir bölüm açar ve altında
baskılar ve sürümler diye bir alt başlık tutar.[^k1] Bu dosya, kuramını değil
**baskılar arasındaki farkı** ele alır.

## Birinci baskı sonrakilerden farklıdır

Kaynak, ikinci ve sonraki baskılarda alt sınıfların yoksulluğunu hafifletmenin en
iyi yolu olarak ahlaki kısıtlamaya daha fazla vurgu yaptığını yazar; birinci
baskının ayrıca insan davranışını açıklamaya çalışan gelişkin bir zihin kuramı
içerdiğini ve bu maddeci zihin anlayışının zihnin bedensel ve akıldışı doğasını
öne çıkardığını kaydeder.[^k1]

Aynı başlığın birinci baskısında bulunan bir bölümün sonraki baskılarda ayrıca
anılmaması,[^k1] "Malthus şöyle der" biçimindeki cümlelerin hangi baskıya
dayandığı belirtilmeden kurulamayacağını gösterir. Aynı sorun
[Thomas Hobbes](/dusunur/hobbes/) ve [Immanuel Kant](/dusunur/kant/)
dosyalarında da görülür.

## Kuramın mekanizması metinde yazılıdır

Kaynak, nüfus ile iktisat arasındaki ilişki hakkında şunu yazdığını kaydeder:
emekçi nüfusu gıda üretiminden hızlı büyüdüğünde, büyüyen nüfus geçim maliyetini
yükselttiği için gerçek ücretler düşer; aile geçindirmenin güçlükleri sonunda
nüfus artış hızını azaltır ve düşen nüfus yeniden daha yüksek gerçek ücretlere yol
açar.[^k1]

Bir kuramın döngüsel olarak kurulması,[^k1] atlasın bunu bir öngörü değil bir
model olarak kaydetmesini gerektirir.

Modelin sonraki nüfus verileriyle ilişkisi için atlasın kavram katmanında ayrı bir
dosya vardır; o dosyanın adı yirminci yüzyılda ortaya çıkmış bir terimdir ve
kullanılan kaynak bölümünde geçmediği için burada anılmaz.[^k1] Bağlantı, bu
dosyanın üst verisindeki ilgili alanında tutulmaktadır.

## Kaynak karşı görüşü de kaydeder

Kaynak, tarımsal iyileştirmelerin sınırsız biçimde genişleyebileceği düşüncesi
gibi çeşitli düşünülebilir çözümlerin bulunduğunu yazar; ayrıca siyasal iktisat
üzerine Ricardo ile yürüttüğü tartışmaya bir bölüm ayırır.[^k1]

Bir kuramın yanına karşı görüşlerin ve bir tartışma başlığının kaydedilmesi,[^k1]
atlasın hakemlik etmeme ilkesinin kaynaklarda da uygulandığını gösterir.

## Dönemin nüfus serisi

Uzun dönemli seride dünya nüfusu 1790'da yaklaşık 942,3 milyon, 1834'te yaklaşık
1,14 milyardır.[^k3]

Atlas bu seriyi kuramın doğrulanması ya da yanlışlanması olarak sunmaz; kuram
tek bir ülkeyle sınırlı bir mekanizma önerirken seri dünya toplamını
verir.[^k1][^k3] İki ölçünün kapsamı aynı değildir.

## Bir önceki kuşak

Bir önceki kuşağın iktisat yazarı için tutulan kayıt, onun ahlak kurallarının
belirli örneklere verilen tepkilerden oluştuğunu savunduğunu yazar.[^k2] O yazar
[Adam Smith](/dusunur/smith/) dosyasında ele alınıyor.

## Bu dosyanın sınırı

Hayatı, Tahıl Yasaları üzerine yazıları, siyasal iktisat ilkeleri ve sonraki
alımlanması burada ayrı başlıklar altında ele alınmamıştır; kaynak bunları ayrı
bölümler olarak işler.[^k1]

## Eserine bağ

Bu dosya düşünürün başlıca eserinin kamu malı tam metnine bağlanır: *Nufus Ilkesi Uzerine Deneme*.[^k4] Esere yapılacak her atıf bölüm ya da paragraf numarasıyla
verilmeli; hangi baskının ve hangi çevirinin kullanıldığı belirtilmelidir.
