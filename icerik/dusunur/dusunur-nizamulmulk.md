---
id: dusunur-nizamulmulk
tip: dusunur
baslik: Nizâmülmülk
ozet: >-
  Selçuklu veziri ve Siyâsetnâme'nin yazarı; yönetme sanatını tarihsel
  örnekler üzerinden anlatan eseri, aynı yıllarda aynı coğrafyada yazılan
  Kutadgu Bilig ile birlikte okunduğunda bir türün iki örneğini verir.
tarih_baslangic: "1018"
tarih_bitis: "1092"
bolge: [orta-asya, islam-dunyasi]
eksen: [siyasi, felsefi, kulturel]
guven_geneli: yaygin
etiketler: [nizamulmulk, siyasetname, selcuklu, medrese, hukumdar-aynasi]
ilgili:
  - aktor-karahanlilar
  - dusunur-gazali
  - dusunur-ibn-sina
  - kavram-burokrasi
  - kavram-mesruiyet
  - kavram-kaynak-elestirisi
okuma_onerisi: []
kaynaklar:
  - anahtar: k1
    tur: ansiklopedi
    ad: "TDV Islam Ansiklopedisi - NIZAMULMULK"
    url: https://islamansiklopedisi.org.tr/nizamulmulk
    erisim_tarihi: 2026-08-25
    dogrulama_dizesi: "NİZÂMÜLMÜLK"
    not: "Turkce akademik ansiklopedi maddesi"
  - anahtar: k2
    tur: ansiklopedi
    ad: "TDV Islam Ansiklopedisi - SIYASETNAME"
    url: https://islamansiklopedisi.org.tr/siyasetname
    erisim_tarihi: 2026-08-25
    dogrulama_dizesi: "SİYÂSETNÂME"
    not: "Eserin ait oldugu tur uzerine ayri madde"
  - anahtar: k3
    tur: birincil
    ad: "The Book of Government, or Rules for Kings — tam metin"
    url: https://archive.org/details/the-book-of-government-or-rules-for-kings
    erisim_tarihi: 2026-08-25
    dogrulama_dizesi: "The Book Of Government"
    not: "Siyasetname'nin Ingilizce cevirisi; katalog yazari Nizam Al-Mulk"
son_denetim: 2026-08-25
denetim_durumu: onaylandi
onarim_turu: 0
---

## Nerede durduğu

Nizâmülmülk, Selçuklu devletinde uzun yıllar vezirlik yapmış bir devlet
adamıdır.[^k1] Atlasın düşünür katmanında yer almasının sebebi idari görevi
değil, yazdığı eserdir: yönetme üzerine kaleme aldığı metin, kendisinden
sonraki siyaset düşüncesinde uzun süre okunmuştur.[^k1][^k2]

Bir devlet görevlisinin aynı zamanda düşünür dosyası taşıması, atlasın tip
ayrımının nasıl işlediğini gösterir. Aktör dosyaları siyasi birimleri kaydeder;
düşünür dosyaları, adı bir metne bağlanabilen kişileri.[^k1] Nizâmülmülk her
iki tarafa da bakar ama dosyası metnin bulunduğu tarafta açılmıştır.

Yaşadığı yıllar, atlasın kronolojik omurgasında yedinci dönemin içine
düşer.[^k1] Aynı dönemde aynı coğrafyada, [Karahanlılar](/aktor/karahanlilar/)
dosyasında ele alınan iki eser de yazılmıştır; bu yakınlık aşağıda ayrıca
işlenir.

## Siyâsetnâme: hangi türden bir metin

Eser, hükümdarın nasıl yönetmesi gerektiğini anlatan bir öğüt kitabıdır.[^k2][^k3]
Bölümler, yönetimin ayrı ayrı alanlarını ele alır: adaletin sağlanması,
görevlilerin denetlenmesi, haber alma düzeni, saray teşkilatı ve benzerleri.[^k3]

Metnin yöntemi tanımlayıcı değil örnekleyicidir. Bir ilke ortaya konur,
ardından o ilkenin geçmişteki uygulamalarından anlatılar aktarılır.[^k3] Bu
yapı, eserin ne olduğunu belirler: bir hukuk derlemesi ya da idari yönetmelik
değil, ikna etmeye çalışan bir metindir.

Atlas bu ayrımı korumak zorundadır. Bir öğüt metninden dönemin fiilî yönetim
pratiği doğrudan okunamaz; metin, yönetimin nasıl olması gerektiğini söyler,
nasıl olduğunu değil.[^k3] Aradaki fark, aynı kaynağın iki farklı soruya
verdiği iki farklı ağırlıktır.

Eserin hükümdarın isteği üzerine yazıldığı kaydedilir.[^k1][^k2] Bir metnin
sipariş üzerine yazılması, içeriğini geçersiz kılmaz ama okunma biçimini
belirler: yazar, muhatabını memnun etme kısıtı altında yazmaktadır.

## Aynı türün iki örneği

Bu dosyanın atlasa kattığı asıl şey, tek bir kişinin kaydı değil bir
karşılaştırma imkânıdır. [Karahanlılar](/aktor/karahanlilar/) dosyasında ele
alınan *Kutadgu Bilig* de bir hükümdara sunulmuş öğüt metnidir ve aynı
yılların ürünüdür.[^k2]

İki metin aynı türe aittir ama farklı dillerde ve farklı siyasi merkezlerde
yazılmıştır.[^k2][^k3] Bu, türün tek bir hanedana ya da tek bir dile bağlı
olmadığını gösterir: yöneticiye öğüt vermek, o yüzyılın siyasi
düşüncesinin ortak biçimidir.

Karşılaştırma, atlasın kavram katmanıyla da bağlanır.
[Meşruiyet](/kavram/mesruiyet/) dosyası, bir yönetimin hangi gerekçelerle
kabul edildiğini sorar; öğüt metinleri bu gerekçelerin dönemin kendi
diliyle nasıl kurulduğunu gösteren birincil malzemedir.[^k3]

İki metnin yan yana okunabilmesi, atlasın bağ yapısının ne işe yaradığının
bir örneğidir. Tek başına her dosya bir kaydı tutar; birlikte okunduklarında
kaydın ötesinde bir soru doğar.

## Medrese düzeni

Nizâmülmülk'ün adıyla anılan medreselerin kurulması, dönemin eğitim
düzeninde belirleyici sayılır.[^k1] Bir devlet görevlisinin eğitim kurumları
kurması, yönetim ile bilgi üretimi arasındaki bağın kurumsallaşmasıdır.

Bu kurumsallaşmanın siyasi bir tarafı da vardır: eğitim kurumlarının kimin
tarafından finanse edildiği, orada hangi görüşlerin öğretileceğini de
etkiler.[^k1] Atlas bu bağı kaydeder ve bir hüküm vermez; kurumların
kuruluş gerekçeleri ile sonuçları ayrı sorulardır.

[Bürokrasi](/kavram/burokrasi/) dosyası, yönetimin kurallara bağlanmasını
kavram düzeyinde ele alır. Bu dosya, o kavramın bir dönemdeki somut
karşılığını verir: görevlilerin denetimi ve haber alma düzeni, Siyâsetnâme'nin
kendi bölümleri arasındadır.[^k3]

:::tartismali
Nizâmiye medreselerinin belirli bir mezhebi yaymak amacıyla mı yoksa genel
bir eğitim ihtiyacını karşılamak için mi kurulduğu tartışmalıdır. İki yorum da
aynı kurumsal kayıtlara dayanır ve aradaki fark, kurucunun niyetine dair bir
çıkarımdan doğar. Atlas bu konuda hüküm vermez.[^k1]
:::

## Gazâlî ile bağlantı

[Gazâlî](/dusunur/gazali/), Nizâmülmülk'ün kurduğu düzen içinde görev
yapmıştır.[^k1] Bu bağ, atlasın iki dosyasını birbirine bağlar ve dönemin
düşünce hayatının kurumsal zeminini gösterir: bir düşünürün nerede
yazdığı, ne yazdığını da etkiler.

Aynı bağ, [İbn Sînâ](/dusunur/ibn-sina/) dosyasında anlatılan felsefe
geleneğiyle Gazâlî'nin itirazı arasındaki tartışmanın hangi kurumsal
ortamda yürüdüğünü de açıklar.[^k1] Tartışmanın metinleri kadar,
tartışmanın yürüdüğü yer de kayda değerdir.

## Kaynak durumu

Bu dosya üç kaynağa dayanır: kişi üzerine bir madde,[^k1] eserin ait olduğu
tür üzerine ayrı bir madde,[^k2] ve eserin İngilizce çevirisi.[^k3] İlk ikisi
aynı alan adındadır; bağımsızlık şartı asgari düzeyde sağlanır ve bu burada
beyan edilir.

Birincil metin bir çeviridir. Özgün metne değil, bir aktarıma bağlanılmıştır;
esere yapılacak atıflarda hangi çevirinin ve hangi bölüm numaralandırmasının
kullanıldığı belirtilmelidir.[^k3]

Eserin farklı yazma nüshaları arasında bölüm sayısı ve içerik bakımından
farklar bulunduğu kaydedilir.[^k2] Bu, tek bir çeviriye bağlanmanın sınırını
gösterir: çeviri, çevirmenin seçtiği nüshayı aktarır ve o seçim metnin
kapsamını belirler.

Bu dosyanın kapsamadıkları da belirtilmelidir. Vezirlik dönemindeki siyasi
olaylar ayrıntılı olarak işlenmez; Selçuklu devlet yapısı ayrı bir aktör
dosyası gerektirir ve bu dosya yazıldığında atlasta bulunmuyordu.[^k1]

## Öğüt metinlerinin tarih kaynağı olarak değeri

Bir öğüt metni, dönemin sorunları hakkında dolaylı bilgi taşır. Yazarın
neyi düzeltilmesi gerektiğini düşündüğü, o şeyin bozuk olduğuna dair bir
işarettir.[^k3] Metnin uzun uzun anlattığı konular, dönemin gündemini
gösterir.

Bu okuma biçimi dikkat ister. Bir metnin bir konuya çok yer ayırması, o
konunun gerçekten sorunlu olduğunu kanıtlamaz; yazarın ilgisini de
gösterebilir.[^k3] Atlas bu ikisini ayrı tutar ve metinden çıkarılan
her iddiayı metnin türüne göre tartar.

Aynı yöntem sorusu, atlasın [Kaynak eleştirisi](/kavram/kaynak-elestirisi/)
dosyasında genel biçimiyle ele alınır. Bu dosya, o sorunun tek bir metin
üzerindeki somut hâlidir.

## Metnin bölümlenişi

Eser, konuları başlıklara ayırarak ilerler ve her başlık kendi içinde
kapanır.[^k3] Bu düzen, metnin baştan sona okunmak zorunda olmadığını
gösterir: bir yönetici, ilgilendiği konunun başlığına doğrudan
bakabilir.

Bölümlerin sırası da bilgi taşır. Adaletle açılan bir metin, yönetimin
meşruiyetini önce adalet üzerinden kuruyor demektir;[^k3] aynı metin
vergiyle açsaydı başka bir öncelik sırası ilan etmiş olurdu.

Atlas bu tür yapısal işaretleri kaydetmeye önem verir. Bir metnin neyi
önce söylediği, ne söylediği kadar bilgi taşır ve bu bilgi metnin
içeriğinden değil düzeninden okunur.[^k3]

Bölümlerin uzunlukları arasındaki farklar da benzer biçimde okunabilir.
Kısa tutulmuş bir konu, yazarın az söyleyecek şeyi olduğunu ya da konuyu
tartışmalı bulduğunu gösterebilir; ikisi arasında karar vermek metnin
kendisinden çıkmaz ve atlas bu tür çıkarımları yapmaz.[^k3]

## Anlatıların kaynak değeri

Metnin ilkelerini örneklediği tarihsel anlatılar, ayrı bir kaynak sorunu
doğurur.[^k3] Bu anlatılar, geçmişte olmuş olayların kaydı olarak değil,
yazarın kendi döneminde dolaşımda olan anlatıların kaydı olarak
değerlidir.

Bir hükümdar hakkında anlatılan bir hikâyenin doğru olup olmadığı ile o
hikâyenin anlatılıyor olması ayrı iki olgudur.[^k3] İkincisi, birincisi
doğrulanamasa bile dönem hakkında bilgi verir: hangi davranışın örnek
sayıldığı, hangi sonun ibret sayıldığı o hikâyelerden okunur.

Bu ayrımın açıkça yapılması, atlasın böyle metinlerden alıntı yaparken
uyguladığı kuraldır. Bir öğüt metninden aktarılan anlatı, olay kaydı
olarak değil dönem tasavvurunun kaydı olarak künyelenir.[^k2][^k3]

Aynı kural, bu dosyanın karşılaştırdığı öteki öğüt metni için de
geçerlidir.[^k2] İki metin de kendi dönemlerinin neyi örnek saydığını
gösterir; ikisi de o dönemin ne yaptığını doğrudan göstermez.

## Eserin sonraki dönemlerde okunması

Metnin kendisinden sonraki yüzyıllarda okunmayı sürdürmesi, türün
kalıcılığının bir işaretidir.[^k2] Bir öğüt metninin uzun süre
okunması, anlattığı sorunların uzun süre var olduğunu da düşündürür.

Ancak bu okuma sürekliliği, metnin her dönemde aynı biçimde anlaşıldığı
anlamına gelmez.[^k2] Aynı metin, farklı siyasi düzenlerde farklı
gerekçelerle okunabilir; okunma sıklığı ile anlaşılma biçimi ayrı ayrı
ölçülmesi gereken şeylerdir.

Atlas bu dosyada metnin sonraki alımlanma tarihini işlemez. O tarih ayrı
bir dosya gerektirir ve mevcut kaynaklarla yazılamaz.[^k1][^k2]

Alımlanma tarihinin ayrı tutulması, atlasın genel bir tercihidir. Bir eserin
kendisi ile o eserin sonraki okunuşları farklı sorulara cevap verir; ikisini
aynı dosyada toplamak, metnin ne dediği ile ne anlaşıldığını birbirine
karıştırma riski taşır.[^k2] Aynı ayrım kaynak dosyalarında da uygulanır.

## Okuma yönlendirmesi

Aynı türün başka bir örneği için [Karahanlılar](/aktor/karahanlilar/)
dosyasında ele alınan *Kutadgu Bilig* okunmalıdır;[^k2] dönemin düşünce
ortamı için [Gazâlî](/dusunur/gazali/) ve [İbn Sînâ](/dusunur/ibn-sina/)
dosyaları aynı zinciri sürdürür.

Kavram tarafından girmek isteyen okuyucu için
[Meşruiyet](/kavram/mesruiyet/) ve [Bürokrasi](/kavram/burokrasi/) dosyaları,
bu metnin konularının bugünkü adlarını verir.

Metnin nasıl okunacağı konusunda bir uyarı gereklidir. Bu dosya eserin ne
yaptığını anlatır, ne dediğini değil; bölümlerin içeriği için metnin kendisine
inilmelidir.[^k3] Bir öğüt metnini özetlemek, onun ikna etme biçimini —
yani asıl özelliğini — silen bir işlemdir.

Aynı uyarı bu dosyanın karşılaştırdığı öteki metin için de geçerlidir.[^k2]
İki eseri yan yana koyan bu dosya, ikisinin de yerine geçmez; karşılaştırmanın
kendisi ancak iki metin okunduktan sonra sınanabilir.
