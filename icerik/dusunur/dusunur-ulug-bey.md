...
kaynaklar:
  - anahtar: k1
    tur: ansiklopedi
    ad: "TDV Islam Ansiklopedisi - ULUG BEY"
    url: https://islamansiklopedisi.org.tr/ulug-bey
    erisim_tarihi: 2026-08-26
    dogrulama_dizesi: "ULUĞ BEY"
    not: "Bolgenin birincil tercih kaynagi"
  - anahtar: k2
    tur: ansiklopedi
    ad: "TDV Islam Ansiklopedisi - ZIC"
    url: https://islamansiklopedisi.org.tr/zic
    erisim_tarihi: 2026-08-26
    dogrulama_dizesi: "ZÎC"
    not: "Uretilen eser turunun kendi maddesi"
  - anahtar: k3
    tur: ansiklopedi
    ad: "Wikipedia (EN) - Ulugh Beg"
    url: https://en.wikipedia.org/wiki/Ulugh_Beg
    erisim_tarihi: 2026-08-26
    dogrulama_dizesi: "Ulugh"
...
hükümdardır.[^k1] Bölgenin kurucu fatihinin torunudur ve yönetimi
gençliğinde devraldı.
...
Semerkant'ta bir rasathane yaptırdı.[^k1] Yapı, dönemin en büyük
gözlem araçlarından birini barındırıyordu: yarıçapı çok büyük, taşa
oturtulmuş bir yay.
...
Aletin büyüklüğü bir tercih değil, bir zorunluluktu.[^k1] O dönemde
açı ölçümünün hassasiyeti, doğrudan aletin boyutuna bağlıydı.

Yay ne kadar uzunsa, bir derecelik açı o kadar geniş bir yay parçasına
...
Rasathanede üretilen eser bir zîctir.[^k2] Zîc, gök cisimlerinin
konumlarını hesaplamaya yarayan tablolar bütünüdür.
...
Eski kataloğu kopyalamak ucuzdu ve kabul görürdü.[^k1] Yeniden ölçmek
pahalıydı ve otoriteye karşı çıkma riski taşıyordu.
...
Çalışma tek kişilik değildi.[^k1] Rasathanede dönemin önde gelen
matematikçileri ve astronomları görev aldı.
...
Hükümdarın gözlemlere doğrudan katıldığı aktarılır.[^k1] Bu, dönemin
hükümdarları için olağan bir uğraş değildi.
...
Zîcte verilen bazı değerler bugünkü ölçümlere yakındır.[^k1] Yılın
uzunluğu ve gök ekseninin eğikliği bunlar arasındadır.
...
Katalogda binin üzerinde yıldızın konumu verilir.[^k1] Konumlar bir
koordinat sisteminde, sayısal olarak kaydedilmiştir.
...
Gözlem yerinin enlemi, neyin görülebileceğini belirler.[^k1] Güney
göğünün bir bölümü Semerkant'tan hiç görünmez.
...
Rasathaneden önce bir medrese yaptırılmıştı.[^k1] Medresede matematik
ve astronomi ağırlıklı bir eğitim verildi.
...
Hükümdar öldürülünce rasathane işlevini yitirdi.[^k1] Kurum bakımsız
kaldı ve zamanla yıkıldı.
...
Rasathane yıkıldı ama zîc kaldı.[^k1] Nüshaları çoğaltıldı ve
bölgenin dışına taşındı.
...
Rasathane taşınamazdı.[^k1] Tablolar taşınabilirdi.
...
Zîc, sonraki yüzyıllarda Avrupa'da da yayımlandı.[^k3] Latinceye
çevrildi ve karşılaştırmalı olarak kullanıldı.
...
Yapının yeri yüzyıllarca bilinmiyordu.[^k1] Yirminci yüzyılın başında
yapılan bir kazıyla ortaya çıkarıldı.
...
Zîc aletin nasıl kullanıldığını anlatır.[^k1] Kazı aletin gerçekten
...
Hükümdarın ölçüm işine ayırdığı zaman eleştirildi.[^k1] Yönetim
işlerinin ihmal edildiği ileri sürüldü.
...
Gök gözlemi soyut bir uğraş değildi.[^k1] Tabloların doğrudan bir
kullanımı vardı: takvim hesabı, ibadet vakitleri ve yön tayini.

Bu kullanım, yatırımın gerekçesini de açıklar.[^k3] Bir hükümdarın
...
Zîcte yalnızca sonuçlar değil, yöntem de verilir.[^k2] Hangi
gözlemin nasıl yapıldığı ve hesabın nasıl yürütüldüğü yazılıdır.
...
Tablolarda sayılar altmış tabanlı bir düzenle yazılır.[^k2] Bu düzen,
açı ve zaman ölçümünde uzun süre standarttı.

Standart olması, karşılaştırmayı mümkün kıldı.[^k1] Farklı yerlerde
...
Kent, uzun mesafeli ticaret yollarının kesiştiği yerdeydi.[^k1] Bu
konum, kaynakların ve kişilerin toplanmasını kolaylaştırdı.
...
Dönemin siyasi olayları ayrıntılı izlenmez.[^k3] Hanedan içi çatışma,
...
Aynı bölgeden ölçümle uğraşan bir başka isim için
[Bîrûnî](/dusunur/biruni/), kaynağın hangi gelenekten geldiğinin
tartışılması için [Kâtib Çelebi](/dusunur/katib-celebi/) dosyaları
uygundur.[^k3]

Kaynakların nasıl sınanacağı için
[Kaynak eleştirisi](/kavram/kaynak-elestirisi/) ve
[Birincil kaynak](/kavram/birincil-kaynak/) dosyaları okunabilir.[^k2]
