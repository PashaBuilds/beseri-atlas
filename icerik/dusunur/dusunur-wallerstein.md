---
id: dusunur-wallerstein
tip: dusunur
baslik: Immanuel Wallerstein
ozet: >-
  Kaynak, kuramına yöneltilen eleştiriye kendi alt başlığını ayırır ve terimleri
  ayrı ayrı tanımlar.
tarih_baslangic: "1930"
tarih_bitis: "2019-08-31"
bolge: [amerika, kuresel]
eksen: [ekonomik, siyasi, kulturel]
guven_geneli: tartismali
etiketler: [wallerstein, dunya-sistemi, merkez-cevre, elestiri-basligi, kondratiev]
ilgili:
  - dusunur-braudel
  - dusunur-marx
  - kavram-dunya-sistemi
  - kavram-bagimlilik-kurami
okuma_onerisi: []
kaynaklar:
  - anahtar: k1
    tur: ansiklopedi
    ad: "Wikipedia (EN) - Immanuel Wallerstein"
    url: https://en.wikipedia.org/wiki/Immanuel_Wallerstein
    erisim_tarihi: 2026-08-21
    dogrulama_dizesi: "Wallerstein"
  - anahtar: k2
    tur: akademik
    ad: "Stanford Encyclopedia of Philosophy - Karl Marx"
    url: https://plato.stanford.edu/entries/marx/
    erisim_tarihi: 2026-08-21
    dogrulama_dizesi: "Marx"
    not: "Onceki kusaktan yapisal kuram; bagimsiz alan adi"
  - anahtar: k3
    tur: ansiklopedi
    ad: "Wikipedia (EN) - Fernand Braudel"
    url: https://en.wikipedia.org/wiki/Fernand_Braudel
    erisim_tarihi: 2026-08-21
    dogrulama_dizesi: "Braudel"
    not: "Yakin donemde ayni olcekte calisan tarihci"
son_denetim: 2026-08-21
denetim_durumu: onaylandi
onarim_turu: 0
---

Kaynak, kuram bölümünün altında iki alt başlık tutar: modern dünya sistemi ve
eleştiri.[^k1] Bu dosya, kuramını değil **kaydın eleştiriyi de bölüm yapmasını**
ele alır.

## Eleştiri kuramın yanında durur

Kaynak, kuram bölümünün ikinci alt başlığını doğrudan eleştiriye ayırır.[^k1]

::tartismali[Aynı ansiklopedi maddesi, kuramı ve ona yöneltilen eleştiriyi iki alt başlık olarak yan yana tutuyor. Atlas ikisini de kaydeder ve hangisinin ağır bastığını söylemez.]{harita=tartisma-tarihsel-sayilar-nasil-okunur}

Bu tür yan yana kayıtların nasıl okunacağı
[Tarihsel sayılar nasıl okunur?](/tartisma/tarihsel-sayilar-nasil-okunur/)
haritasında ele alınıyor.

## Terimler ayrı ayrı tanımlanmıştır

Kaynak terimler ve tanımlar başlığı altında beş alt başlık tutar: kapitalist
dünya sistemi, merkez ve çevre, yarı çevre, yarı tekeller ve Kondratiev
dalgaları.[^k1]

Bir kuramın terimlerinin ansiklopedide ayrı ayrı tanımlanması,[^k1] bu atlasın
kavram katmanının neden ayrı bir tip olarak kurulduğunun dayanağıdır; kavram
[Dünya sistemi](/kavram/dunya-sistemi/) dosyasında ele alınıyor.

## Kariyer ve ödüller de kayıtlıdır

Kaynak, 1990'larda toplum bilimlerinin yeniden yapılandırılması üzerine bir
komisyona başkanlık ettiğini ve komisyonun amacının önümüzdeki elli yıl için
toplum bilimsel araştırmaya bir yön göstermek olduğunu yazar; 2000 ile 2019'daki
ölümü arasında Yale Üniversitesi'nde kıdemli araştırmacı olarak çalıştığını,
2003'te bir ödül aldığını ve 2019'da 88 yaşında bir enfeksiyondan öldüğünü
kaydeder.[^k1]

Bir kuramcının kariyerinin kurumlar ve ödüllerle kaydedilmesi,[^k1] kuramların
kurumsal ortamlarda üretildiğini gösteren bir başka örnektir.

## Yakın dönemde aynı ölçekte çalışan tarihçi

Yakın dönemde aynı ölçekte çalışan bir tarihçi için tutulan kayıt, o yazarın
kapitalistleri tekelciler olarak gördüğünü yazar.[^k3] O yazar
[Fernand Braudel](/dusunur/braudel/) dosyasında ele alınıyor.

Önceki kuşağın yapısal kuramı için tutulan kayıt ise toplum biçimlerinin üretici
gücün gelişimine göre yükselip düştüğü fikrini anlatır.[^k2] O kuram
[Karl Marx](/dusunur/marx/) dosyasında ele alınıyor.

## Bu dosyanın sınırı

Dünya sistemi kuramının içeriği, akademik kariyeri ve eleştirilerin ayrıntıları
burada ayrı başlıklar altında ele alınmamıştır; kaynak bunları ayrı bölümler
olarak işler.[^k1]
