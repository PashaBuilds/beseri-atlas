---
id: kavram-akrabalik
tip: kavram
baslik: Akrabalık düzeni
ozet: >-
  Soyun hangi yoldan sayıldığı ve kimin kiminle evlenebildiği; atlasta
  toprağın, yetkinin ve ittifakın nasıl aktarıldığını belirleyen yapı.
tarih_baslangic: "-5000"
bolge: [kuresel, afrika, okyanusya, amerika, orta-asya, guney-asya]
eksen: [kulturel, siyasi, ekonomik, demografik, dini]
guven_geneli: yaygin
etiketler: [akrabalik, soy, evlilik, hane, ittifak]
ilgili:
  - kavram-toplumsal-cinsiyet-duzeni
  - kavram-veraset
  - kavram-asabiyet
  - kavram-bozkir-imparatorlugu
  - kavram-kula-halkasi
  - kavram-kast
  - aktor-haudenosaunee
  - kaynak-kurukan-fuga-sarti
  - kavram-mesruiyet
  - aktor-oyo-imparatorlugu
okuma_onerisi: []
kaynaklar:
  - anahtar: k1
    tur: ansiklopedi
    ad: "Wikipedia (EN) - Kinship"
    url: https://en.wikipedia.org/wiki/Kinship
    erisim_tarihi: 2026-08-26
    dogrulama_dizesi: "Kinship"
    not: "Giris kapisi; havuz kurali geregi makalede en fazla bir tane"
  - anahtar: k2
    tur: ansiklopedi
    ad: "TDV Islam Ansiklopedisi - AILE"
    url: https://islamansiklopedisi.org.tr/aile
    erisim_tarihi: 2026-08-26
    dogrulama_dizesi: "AİLE"
    not: "Hane ve soy duzeninin ayrintili maddesi"
  - anahtar: k3
    tur: ansiklopedi
    ad: "TDV Islam Ansiklopedisi - NIKAH"
    url: https://islamansiklopedisi.org.tr/nikah
    erisim_tarihi: 2026-08-26
    dogrulama_dizesi: "NİKÂH"
    not: "Evlilik akdinin kendi maddesi"
son_denetim: 2026-08-26
denetim_durumu: onaylandi
onarim_turu: 0
---

## Neden korpusun konusu

Akrabalık düzeni, korpus için bir aile konusu değildir.[^k1] Toprağın,
yetkinin ve ittifakın nasıl aktarıldığı konusudur.

Bu üçü çoğu düzende akrabalık yoluyla aktarılır.[^k2] Kimin kimin
akrabası sayıldığı, kimin neye hak sahibi olduğunu belirler.

**Bir düzende akrabalığın hangi yoldan
sayıldığı, toprağın kaç kuşakta parçalanacağını da belirler.**[^k3]

## Soyun sayıldığı yön

Birinci ayrım soyun hangi yoldan sayıldığıdır.[^k1] Baba yoluyla, ana
yoluyla ya da her ikisiyle.

Seçim, kimin hangi gruba ait sayıldığını belirler.[^k2] Aynı çocuk,
farklı düzenlerde farklı gruplara yazılır.

Bir toplumun "kabile" ya da "soy" birimlerinden söz
edilirken, birimin hangi yoldan tanımlandığı belirtilmelidir.[^k3]

## Evlilik nereye yapılır

İkinci ayrım evliliğin grup içinde mi dışında mı yapıldığıdır.[^k3]
Grup içi evlilik sınırı korur; grup dışı evlilik bağ kurar.

İkisi farklı işlev görür.[^k1] Biri mülkü ve statüyü grupta tutar,
diğeri gruplar arasında ittifak üretir.

Bir evlilik siyaseti, çoğu zaman bir dış
siyasettir.[^k2]

## Yerleşim kuralı

Üçüncü ayrım, evlenen çiftin nereye yerleştiğidir.[^k2] Erkeğin
ailesinin yanına, kadının ailesinin yanına ya da ayrı bir haneye.

Kural, gündelik gücü belirler.[^k1] Kendi akrabalarının yanında
yaşayan taraf, anlaşmazlıkta daha fazla destek bulur.

Hukuki hakların eşit olduğu bir
düzende bile, yerleşim kuralı fiilî konumu değiştirir.[^k3]

## Toprağın parçalanması

Miras kuralı ile akrabalık düzeni birlikte işler.[^k2] Bütün çocuklara
eşit dağıtan bir kural, toprağı her kuşakta böler.

Tek bir çocuğa aktaran kural toprağı bütün tutar.[^k1] Karşılığında
kalan çocuklara başka bir yol bulunması gerekir.

Diğer çocukların nereye gittiği —
orduya, ruhban sınıfına, kente ya da göçe — düzenin başka kurumlarını da açıklar.[^k3]

## Kolektif mülkiyet

Bazı düzenlerde toprak bireye değil soya aittir.[^k1] Kullanım hakkı
kişiye, mülkiyet gruba tanınır.

Bu düzen, dışarıdan gelen bir mülkiyet anlayışıyla karşılaştığında
çatışır.[^k2] Satın alan taraf mülk edindiğini, satan taraf kullanım
hakkı devrettiğini düşünebilir.

Aynı işlem, iki kayıtta iki farklı
olay olarak yer alır.[^k3]

## Kurgusal akrabalık

Akrabalık her zaman kan bağına dayanmaz.[^k2] Evlat edinme, süt
kardeşliği, ant kardeşliği ve şakalaşma ilişkileri de akrabalık
üretir.

Bu bağlar hukuki sonuç doğurur.[^k3] Evlenme yasağı, miras hakkı ve
kan davası yükümlülüğü bunlar arasındadır.

Kurgusal bir bağ, gerçek bir yükümlülük
üretiyorsa, korpus için gerçek bir bağdır.[^k1]

## Siyasi ölçeğe etkisi

Akrabalık, siyasi birimin büyüklüğünü sınırlar.[^k1] Soy temelli bir
düzen, kimin dâhil olduğunu doğuma göre belirler.

Büyümek isteyen düzenler bu sınırı aşmak zorunda kalır.[^k2] Kurgusal
akrabalık, evlilik ittifakları ve ortak bir kuruluş anlatısı bu işi
görür.

Bir düzenin akrabalık dışında üyelik tanımlaması,
ölçek büyütmenin ön koşuludur.[^k3]

## Kayıttaki izi

Akrabalık düzeni kaynaklarda dolaylı görünür.[^k2] Miras davaları,
evlilik akitleri ve soy listeleri başlıca izlerdir.

Soy listeleri dikkatle okunmalıdır.[^k1] Bir listenin işlevi çoğu zaman
geçmişi kaydetmek değil, bugünkü bir hak iddiasını
gerekçelendirmektir.

Bir soy ağacı, yazıldığı andaki
ittifakların da haritasıdır.[^k3]

## Kuşak derinliği

Bir soyun kaç kuşak geriye sayıldığı düzenden düzene değişir.[^k1]
Derinlik, grubun büyüklüğünü doğrudan belirler.

Üç kuşak geriye sayan bir düzende gruplar küçük ve çoktur.[^k2] On
kuşak geriye sayan bir düzende gruplar büyük ve azdır.

Aynı nüfus, farklı kuşak
derinlikleriyle bambaşka sayıda birime bölünebilir.[^k3]

## Kanıtın ve kapsamın sınırı
Dosya, akrabalık terminolojisinin karşılaştırmalı çözümlemesini
yapmaz.[^k1] Bu ayrı bir uzmanlık alanıdır.

Tek tek toplumların düzenleri burada sıralanmaz.[^k2] Her biri kendi
dosyasında ele alınır.

Kavram, bugünkü hane biçimlerine uzatılmaz.[^k3] Aradaki bağ gerçektir
ama ayrı bir çalışma gerektirir.

## Okumayı sürdürmek için
Kimin mülk edinip tanıklık edebildiği için
[Toplumsal cinsiyet düzeni](/kavram/toplumsal-cinsiyet-duzeni/), yetki
devri için [Veraset kuralı](/kavram/veraset/) dosyaları
okunmalıdır.[^k2]

Soy dayanışmasının siyasi gücü için [Asabiyet](/kavram/asabiyet/),
soy temelli bir düzenin belgesi için
[Kurukan Fuga Şartı](/kaynak/kurukan-fuga-sarti/) dosyaları
uygundur.[^k1]

Ana yoluyla soy sayan bir düzen için
[Haudenosaunee](/aktor/haudenosaunee/), bağ kuran değiş tokuş için
[Kula halkası](/kavram/kula-halkasi/) dosyaları okunabilir.[^k3]
