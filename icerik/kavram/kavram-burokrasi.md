---
id: kavram-burokrasi
tip: kavram
baslik: Bürokrasi
ozet: >-
  Kaynak tarihini antik Çin ve antik Roma ile başlatır; kavramın adı modern
  olsa da örnekleri değildir.
bolge: [kuresel]
eksen: [siyasi, ekonomik]
guven_geneli: yaygin
etiketler: [burokrasi, antik-cin, kuram, adlandirma, modern]
ilgili:
  - dusunur-weber
  - kavram-imparatorluk
  - aktor-han-imparatorlugu
  - kavram-patrimonyalizm
  - kavram-vergi
okuma_onerisi: []
kaynaklar:
  - anahtar: k1
    tur: ansiklopedi
    ad: "Wikipedia (EN) - Bureaucracy"
    url: https://en.wikipedia.org/wiki/Bureaucracy
    erisim_tarihi: 2026-08-21
    dogrulama_dizesi: "Bureaucracy"
  - anahtar: k2
    tur: akademik
    ad: "Stanford Encyclopedia of Philosophy - Karl Marx"
    url: https://plato.stanford.edu/entries/marx/
    erisim_tarihi: 2026-08-21
    dogrulama_dizesi: "Marx"
    not: "Kaynagin kuramlar bolumunde ilk sirada saydigi ad; bagimsiz alan adi"
  - anahtar: k3
    tur: ansiklopedi
    ad: "Wikipedia (EN) - Max Weber"
    url: https://en.wikipedia.org/wiki/Max_Weber
    erisim_tarihi: 2026-08-21
    dogrulama_dizesi: "Weber"
    not: "Kavrami kendi kuram listesinde tutan yazar"
son_denetim: 2026-08-21
denetim_durumu: onaylandi
onarim_turu: 0
---

Kaynak, tarih bölümünü antik ve modern diye ikiye ayırır; antik başlığı altında
antik Çin ile antik Roma'yı, modern başlığı altında İran, Rusya, Aşanti
İmparatorluğu, Birleşik Krallık, Fransa ile sanayileşmiş ve küreselleşmiş dünyayı
ayrı alt başlıklarda tutar.[^k1]

## Kavramın adı modern, örnekleri değildir

Kaynağın ilk bölümünü köken bilgisi ve kullanıma ayırması ve tarihi antikçağla
başlatması,[^k1] atlasın şu kuralının dayanağıdır: bu atlasta bürokrasi terimi,
antik dönem dosyalarında geriye dönük bir adlandırma olduğu belirtilerek
kullanılır. Aynı sorun
[Anakronizm](/kavram/anakronizm/) dosyasında ele alınıyor.

## Aşanti İmparatorluğu modern başlığı altındadır

Kaynağın modern örnekler arasında Aşanti İmparatorluğu'nu sayması,[^k1] kavramın
Avrupa devletlerine özgü sayılmasının kaynağın kendi bölümlemesince
desteklenmediğini gösterir.

## Kuramlar ad ad sayılır

Kaynak, kuramlar başlığı altında Karl Marx ve Leon Troçki gibi adları ayrı alt
başlıklar olarak tutar.[^k1] Bunlardan biri için tutulan ayrı kayıt, o düşünürü
1818-1883 arasına yerleştirir.[^k2]

Bir yönetim biçiminin kuramlarının kişi adlarıyla bölümlenmesi,[^k1][^k2] atlasın
kavram dosyalarında neden düşünür bağlantısı tuttuğunun dayanağıdır; o düşünür
[Karl Marx](/dusunur/marx/) dosyasında ele alınıyor.

## Kavramı kendi listesinde tutan yazar

Kavramı kendi kayda değer fikirler listesinde tutan bir yazar için ayrı bir kayıt
bulunur; aynı kayıt onu 1864-1920 arasına yerleştirir ve otoritenin üçlü
sınıflandırmasını karizmatik, akılcı-yasal ve geleneksel olarak verir.[^k3] O
yazar [Max Weber](/dusunur/weber/) dosyasında ele alınıyor.

## Bu dosyanın sınırı

Kavramın tanımı, ülke ülke tarihler ve kuramların içeriği burada ayrı başlıklar
altında ele alınmamıştır; kaynak bunları ayrı bölümler olarak işler.[^k1]
