---
id: kavram-mandala-devleti
tip: kavram
baslik: Mandala devleti
ozet: >-
  Bağlılık ilişkisinin dışlayıcı olmadığı siyasi biçim; bir devlet aynı anda iki
  ya da üç güce haraç ödeyebilir.
bolge: [guney-asya, dogu-asya]
eksen: [siyasi, kulturel]
guven_geneli: yaygin
etiketler: [mandala, dislayici-olmama, harac, sinir, srivijaya]
ilgili:
  - aktor-srivijaya
  - kavram-haracguzarlik
  - tartisma-imparatorluk-siniri-nasil-cizilir
  - kavram-sehir-devleti
  - aktor-malaka-sultanligi
okuma_onerisi: []
kaynaklar:
  - anahtar: k1
    tur: ansiklopedi
    ad: "Wikipedia (EN) - Mandala (political model)"
    url: https://en.wikipedia.org/wiki/Mandala_(political_model)
    erisim_tarihi: 2026-08-21
    dogrulama_dizesi: "Mandala"
  - anahtar: k2
    tur: ansiklopedi
    ad: "Wikipedia (EN) - Srivijaya"
    url: https://en.wikipedia.org/wiki/Srivijaya
    erisim_tarihi: 2026-08-21
    dogrulama_dizesi: "Srivijaya"
    not: "Kaynagin ornek verdigi devlet"
  - anahtar: k3
    tur: akademik
    ad: "Stanford Encyclopedia of Philosophy - Thomas Hobbes"
    url: https://plato.stanford.edu/entries/hobbes/
    erisim_tarihi: 2026-08-21
    dogrulama_dizesi: "Hobbes"
    not: "Mutlak egemenlik karsit ornegi; bagimsiz alan adi"
son_denetim: 2026-08-21
denetim_durumu: onaylandi
onarim_turu: 0
---

Kaynak, dışlayıcı olmama diye ayrı bir bölüm tutar ve efendi-haraçgüzar
ilişkisinin zorunlu olarak dışlayıcı olmadığını yazar; sınır bölgelerindeki bir
devletin iki ya da üç güçlü güce haraç ödeyebileceğini kaydeder.[^k1]

## Bağlılık tekil değildir

Kaynak, haraç ödeyen hükümdarın güçlü güçleri birbirine karşı oynayarak her
birinin müdahalesini en aza indirebileceğini yazar.[^k1]

Bir siyasi birimin aynı anda birden çok merkeze bağlı olabilmesi,[^k1] atlasın
şu kuralının dayanağıdır: bu dosyada bağlılık, bir aidiyet değil bir ilişki
demeti olarak kaydedilir. Harita gösterimi sorunu
[İmparatorluk sınırı haritada nasıl gösterilmeli?](/tartisma/imparatorluk-siniri-nasil-cizilir/)
haritasında ele alınıyor.

## Kaynak somut bir örnek verir

Kaynak, Srivijaya'lı Dharmasetu ile Sailendra'lı Samaratungga arasındaki
ilişkinin bu hanedan ailesinin veraset düzenini tanımladığını yazar; Dharmasetu'nun
Srivijaya maharacası olduğunu, Samaratungga'nın Dharmasetu'nun kızı Prenses Tara
ile evlenmesinin ardından onun halefi olduğunu ve Srivijaya'nın merkezinin bir
yüzyıl boyunca Sumatra'dan Java'ya kaydığını kaydeder.[^k1]

Bir siyasi ilişkinin bir evlilikle tarihlenmesi,[^k1] mandala düzeninde bağlılığın
kurumsal değil kişisel bağlarla kurulduğunu gösteren bir kayıttır; kaynak bunun
için ayrıca kişisel ilişkiler diye bir bölüm tutar.[^k1] Örnek devlet
[Srivijaya](/aktor/srivijaya/) dosyasında ele alınıyor.[^k2]

## Karşıt biçim

Karşıt biçim için tutulan kayıt, mutlak egemenliği savunan bir düşünürü ve onun
1588-1679 arasında yaşadığını verir.[^k3] O düşünür
[Thomas Hobbes](/dusunur/hobbes/) dosyasında ele alınıyor.

İki kaydın yan yana durması,[^k1][^k3] atlasın şu kuralının dayanağıdır:
egemenliğin tekil ve bölünmez sayılması, bu atlasta belirli bir geleneğe ait bir
varsayım olarak kaydedilir, ortak bir başlangıç noktası olarak değil.

## Bu dosyanın sınırı

Terminoloji, kavramın tarihi ve yükümlülüklerin ayrıntısı burada ayrı başlıklar
altında ele alınmamıştır; kaynak bunları ayrı bölümler olarak işler.[^k1]
