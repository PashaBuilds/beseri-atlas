---
id: kavram-svahili-kiyisi
tip: kavram
baslik: Svahili kıyısı
ozet: >-
  Doğu Afrika kıyısında, birbirine bağlı ama tek bir devlet altında
  birleşmemiş liman şehirleri kuşağı; ne Afrika'nın ne de okyanusun tek
  başına açıkladığı bir oluşum.
bolge: [afrika, islam-dunyasi]
eksen: [ekonomik, kulturel, dini, siyasi]
guven_geneli: tartismali
etiketler: [svahili, kilve, mombasa, liman, kiyi, karma-kultur]
ilgili:
  - aktor-kilva
  - kavram-hint-okyanusu-ticareti
  - aktor-buyuk-zimbabve
  - kavram-ticaret-diasporasi
  - dusunur-ibn-battuta
  - kavram-arkeolojik-kanit
  - tartisma-tarihsel-sayilar-nasil-okunur
  - aktor-maldiv-sultanligi
okuma_onerisi: []
kaynaklar:
  - anahtar: k1
    tur: ansiklopedi
    ad: "TDV Islam Ansiklopedisi - KILVE"
    url: https://islamansiklopedisi.org.tr/kilve
    erisim_tarihi: 2026-08-25
    dogrulama_dizesi: "KİLVE"
    not: "Kusagin en cok kaydedilen sehri uzerine akademik madde"
  - anahtar: k2
    tur: ansiklopedi
    ad: "TDV Islam Ansiklopedisi - MOMBASA"
    url: https://islamansiklopedisi.org.tr/mombasa
    erisim_tarihi: 2026-08-25
    dogrulama_dizesi: "MOMBASA"
    not: "Ayni kusagin ikinci sehri; kusagin cokluk yapisini gostermek icin"
  - anahtar: k3
    tur: birincil
    birincil_tur: tanik
    ad: "Ibn Battuta — Travels in Asia and Africa, Gibb cevirisi (1929)"
    url: https://archive.org/details/dli.ernet.16353
    erisim_tarihi: 2026-08-25
    dogrulama_dizesi: "Ibn Battuta"
    not: "Kiyiyi yerinde gormus tek erisilebilir tanik kaydi"
  - anahtar: k4
    tur: ansiklopedi
    ad: "Wikipedia (EN) - Swahili coast"
    url: https://en.wikipedia.org/wiki/Swahili_coast
    erisim_tarihi: 2026-08-25
    dogrulama_dizesi: "Swahili coast"
    not: "Giris kapisi; havuz kurali geregi makalede en fazla bir tane"
son_denetim: 2026-08-25
denetim_durumu: onaylandi
onarim_turu: 0
---

## Ne olduğu

Svahili kıyısı, Doğu Afrika'nın okyanusa bakan şeridinde kurulmuş liman
şehirleri kuşağına verilen addır.[^k1][^k4] Şehirler birbirine ticaret, dil ve
evlilik yoluyla bağlıydı ama tek bir devlet altında birleşmediler.

Kuşağın ayırt edici yanı, siyasi birlik olmadan kültürel birlik
taşımasıdır.[^k2][^k4] Ortak bir dil, ortak bir yapı geleneği ve ortak bir
hukuk çerçevesi, merkezî bir yönetim olmadan sürdürülmüştür.

Bu, [Hint Okyanusu ticareti](/kavram/hint-okyanusu-ticareti/) incelemesinde tanımlanan düzenin batı ucudur.[^k3] Kuşağın varlık sebebi, mevsim
rüzgârlarının bu kıyıyı okyanus ağının başlangıç noktası yapmasıdır.

## Karma bir oluşum

Kuşağın kültürü, ne yalnızca iç bölgeden ne de yalnızca okyanus ötesinden
gelir.[^k1][^k4] Dil, iç bölge dil ailesine ait bir gövde üzerine okyanus
ticaretinin taşıdığı sözcüklerin eklenmesiyle oluşmuştur.

Bu karma yapı, kuşağın nasıl adlandırılacağı sorusunu doğurur.[^k4] Kıyı,
Afrika tarihinin bir bölümü müdür yoksa okyanus ağının bir uzantısı mı?

::tartismali[Kuşağın kökeni tartışmalıdır: bir görüş onu iç bölgeden kıyıya
uzanan yerli bir gelişme sayar, öteki görüş okyanus ötesinden gelen tüccar
yerleşimlerinin ürünü sayar. Arkeolojik bulgular ile yazılı kayıtlar bu
soruda aynı yöne işaret etmez.]{harita=tartisma-imparatorluk-siniri-nasil-cizilir}

## Şehirlerin çokluğu

Kuşakta tek bir merkez yoktur ve bu, kuşağı anlamanın anahtarıdır.[^k1][^k2]
Şehirler zaman zaman birbirine üstünlük kurdu ama hiçbiri kalıcı bir üstünlük
sağlayamadı.

Çokluğun sebebi coğrafyadır. Her liman kendi başına okyanusa açılabildiği
için, hiçbiri ötekine bağımlı değildi;[^k4] bir şehri denetlemek, komşusunu
denetlemeyi getirmiyordu.

Çokluğun sonucu dayanıklılıktır. Bir şehrin düşmesi kuşağı ortadan
kaldırmaz; ticaret komşu limana kayar ve düzen yeniden kurulur.[^k1][^k2]

## Kaynak durumu ve kanıt çelişkisi

Bu kuşak hakkındaki bilginin iki ayrı kaynağı vardır ve ikisi aynı şeyi
söylemez.[^k3][^k4] Yazılı kayıtların çoğu dışarıdan gelen tüccar ve
gezginlerin elinden çıkmıştır; arkeolojik bulgular ise yerleşimlerin
kendisinden gelir.

Yazılı kayıtlar okyanus bağlantısını öne çıkarır; kazılar ise iç bölgeyle
sürekli bir alışverişi gösterir.[^k4] İki kanıt türü çelişmez ama farklı
ağırlıklar verir.

Korpusun [Arkeolojik kanıt](/kavram/arkeolojik-kanit/) dosyası bu gerilimi
genel biçimiyle ele alır. Burada somut hâli şudur: kuşağın kökeni sorusu,
hangi kanıt türüne ağırlık verildiğine göre farklı yanıtlanır.[^k1]

## Tek tanık kaydının ağırlığı

Kuşağı yerinde görmüş ve erişilebilir bir kayıt bırakmış tanık sayısı
azdır.[^k3] Bu incelemede kullanılan tanık kaydı, kıyıdaki iki şehri ziyaret
etmiş bir gezginin anlatısıdır.

Tek bir tanığa dayanmanın riski açıktır. Gezginin gördüğü şehirler kuşağın
tamamını temsil etmeyebilir ve gördüklerini kendi ölçütleriyle
değerlendirmiştir;[^k3] zenginlik, dindarlık ve düzen hakkındaki
değerlendirmeleri karşılaştırmalıdır, ölçüm değildir.

Kaydın değeri, kuşağın o tarihte
işler durumda olduğunu ve dışarıdan gelen birine nasıl göründüğünü
belgelemesindedir.[^k4]

## İç bölgeyle ilişki

Kuşağın okyanusa sattığı malların bir bölümü iç bölgeden geliyordu.[^k1][^k4]
Bu, kıyı şehirlerini yalnızca deniz ticaretinin değil, kıta içi ticaret
ağlarının da ucu yapar.

İç bölgedeki siyasi düzenlerle ilişki, [Büyük Zimbabve](/aktor/buyuk-zimbabve/) incelemesiyle kesişir.[^k4] İki dosya
birlikte okunduğunda, kıyının iç bölgesiz açıklanamayacağı görünür.

Bu bağlantının ayrıntısı bilinmemektedir ve korpus bir güzergâh
çizmez.[^k1][^k4] İç bölge ile kıyı arasındaki aracıların kim olduğu, elde
bulunan kaynaklarla karara bağlanamaz.

## Bağlantılar ve karşılaştırmalar
Bu inceleme, korpusun Doğu Afrika kapsamındaki ilk kavram dosyasıdır.[^k1]
Bölgenin aktör katmanında dosyalar bulunuyordu; bunları birbirine bağlayan
bir yapı tanımı yoktu.

Dosyanın kapsamadıkları belirtilmelidir. Tek tek şehirlerin siyasi tarihi,
hanedanları ve birbirleriyle çatışmaları burada işlenmez;[^k2] her biri ayrı
dosya gerektirir.

Kuşağın Portekiz donanmalarıyla karşılaşması ve sonraki sömürge dönemi de
kapsam dışıdır.[^k4] Bu inceleme kuşağın kendi işleyişini anlatır; sonunu
değil.

## Taş yapı bir kayıt biçimidir

Kuşaktaki şehirlerin taş yapı geleneği, korpusun kanıt katmanı açısından ayrıca
kaydedilmelidir.[^k1][^k4] Mercan taşından yapılmış cami, saray ve mezar
yapıları, yazılı kaydın bulunmadığı yerde bir yerleşimin varlığını, süresini
ve zenginlik derecesini gösterir.

Taş yapının anlattığı şey mimari değildir. Bir yerleşimin taşa geçmesi,
sürekli bir işgücü, sürekli bir gelir ve gelecekte de orada bulunma beklentisi
gerektirir;[^k2] geçici bir ticaret durağı taş yapı kurmaz.

Bu, korpusun bölge dosyalarında tekrar eden bir ölçüttür. Bir düzenin ne kadar
yerleşik olduğu, bıraktığı yapının cinsinden okunabilir ve bu okuma yazılı
kayıttan bağımsız bir kanıt sağlar.[^k4]

## Okumayı sürdürmek için
Kuşağın en çok kaydedilen şehri için [Kilva](/aktor/kilva/), bağlı olduğu
deniz ağı için [Hint Okyanusu ticareti](/kavram/hint-okyanusu-ticareti/)
dosyaları okunmalıdır.

İç bölgedeki siyasi düzen için [Büyük Zimbabve](/aktor/buyuk-zimbabve/),
kıyıyı yerinde gören tanık için [İbn Battûta](/dusunur/ibn-battuta/)
dosyaları uygundur.

Yazılı kayıt ile kazı bulgusu arasındaki gerilim için
[Arkeolojik kanıt](/kavram/arkeolojik-kanit/), kaynaklarda verilen sayıların
nasıl okunacağı için
[Tarihsel sayılar nasıl okunur](/tartisma/tarihsel-sayilar-nasil-okunur/)
dosyaları gereklidir.
