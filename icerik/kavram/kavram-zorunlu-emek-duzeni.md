---
id: kavram-zorunlu-emek-duzeni
tip: kavram
baslik: Zorunlu emek düzeni
ozet: >-
  Emeğin köleliğe çevrilmeden ve ücrete bağlanmadan tahsis edildiği ara
  biçimler; atlasta bir düzenin adının hukuki konumunu belirlediği
  örnek.
tarih_baslangic: "1503"
bolge: [amerika, kuresel, afrika, guney-asya, okyanusya]
eksen: [ekonomik, siyasi, dini, demografik]
guven_geneli: yaygin
etiketler: [encomienda, mita, zorunlu-emek, tahsis, somurge-duzeni]
ilgili:
  - donem-10
  - donem-11
  - dusunur-las-casas
  - aktor-ispanyol-imparatorlugu
  - kaynak-guaman-poma-nueva-coronica
  - olay-tupac-amaru-ayaklanmasi-1780
  - kavram-pasifik-isci-toplama
  - olay-atlantik-kole-ticareti
  - kavram-pamuk-tek-urun
  - tartisma-somurgeciligin-ekonomik-bilancosu
okuma_onerisi: []
kaynaklar:
  - anahtar: k1
    tur: ansiklopedi
    ad: "Wikipedia (EN) - Encomienda"
    url: https://en.wikipedia.org/wiki/Encomienda
    erisim_tarihi: 2026-08-26
    dogrulama_dizesi: "Encomienda"
    not: "Giris kapisi; havuz kurali geregi makalede en fazla bir tane"
  - anahtar: k2
    tur: birincil
    birincil_tur: tanik
    ad: "Guaman Poma - Primer Nueva Coronica y Buen Gobierno, Vol 1"
    url: https://archive.org/details/primer-nueva-coronica-y-buen-gobierno-vol-1
    erisim_tarihi: 2026-08-26
    dogrulama_dizesi: "Coronica"
    not: "Duzeni tabi kilinan tarafin konumundan anlatan cizimli el yazmasi"
  - anahtar: k3
    tur: akademik
    ad: "Stanford Encyclopedia of Philosophy - Colonialism"
    url: https://plato.stanford.edu/entries/colonialism/
    erisim_tarihi: 2026-08-26
    dogrulama_dizesi: "Colonialism"
    not: "Hukuki cerceve"
son_denetim: 2026-08-26
denetim_durumu: onaylandi
onarim_turu: 0
---

## Tanım

Zorunlu emek düzeni, emeğin ne köle mülkiyetiyle ne de serbest sözleşmeyle
sağlandığı ara biçimlerin ortak adıdır.[^k1] Çalışan kişi hukuken
mülk değildir ama çalışmayı reddedemez.

Ara biçimler farklı adlar taşır.[^k2] Kıtanın kuzeyinde ve güneyinde
farklı düzenlemeler farklı adlarla kuruldu.

Atlas bu adları bir arada tutar.[^k3] Ortak nokta, çalışmanın hukuki
olarak zorunlu ama biçimsel olarak kölelik dışı sayılmasıdır.

## Birinci biçim: tahsis

Birinci biçimde, belirli bir topluluğun emeği bir yerleşimciye tahsis
edilir.[^k1] Tahsis toprak mülkiyeti değildir; kişiler üzerinde bir
haktır.

Karşılığında yerleşimciye iki yükümlülük verilir.[^k3] Topluluğu
korumak ve dinî eğitimini sağlamak.

Atlas bu karşılıklılığı ayrıca kaydeder.[^k2] Yükümlülüğün metinde
bulunması, yerine getirildiğini göstermez; iki veri ayrı ayrı
izlenmelidir.

## İkinci biçim: dönüşümlü angarya

İkinci biçimde topluluk, belirli aralıklarla belirli sayıda kişiyi
belirli bir süre için çalışmaya gönderir.[^k1] Kişi değişir, yükümlülük
kalır.

Bu biçim, fetihten önceki bir uygulamanın devamı olarak
sunuldu.[^k2] Ad ve çerçeve korundu, amaç değiştirildi.

Atlas bu devralmayı ölçüm sayar.[^k3] **Yerleşik bir yükümlülüğün adını
koruyup içeriğini değiştirmek, yeni bir yükümlülük kurmaktan daha az
direnç doğurur.**

## Adın işlevi

Bu düzenlerin köleliğe çevrilmemesi tesadüf değildir.[^k1] Kölelik,
dönemin hukuki ve dinî tartışmasında ayrı bir konuma sahipti.

Ara biçim, bu tartışmayı doğrudan karşılamadan aynı emeği
sağlıyordu.[^k3] Hukuki konum değişince gerekçe yükü de değişir.

Atlas bu işlevi ayrıca kaydeder.[^k2] Bir düzene verilen ad, o düzenin
hangi itirazlara cevap vermek zorunda olduğunu belirler.

## Ölçüt

Kavram üç ölçütle uygulanır.[^k1] Birincisi, çalışmayı reddetmenin
mümkün olup olmadığıdır.

İkincisi, kişinin hukuken satılabilir olup olmadığıdır.[^k3]
Üçüncüsü, karşılığın ödenip ödenmediği ve ödeniyorsa kim tarafından
belirlendiğidir.

Üçü birlikte bakılır.[^k2] Ücret ödenmesi, düzeni gönüllü kılmaz;
belirleyici olan reddin mümkün olup olmadığıdır.

## Madencilikle bağlantı

Dönüşümlü angarya en yoğun biçimde madencilikte kullanıldı.[^k2] Yüksek
rakımdaki büyük gümüş ve cıva madenleri bu düzenle işletildi.

Madencilik, ölüm oranı yüksek bir iştir.[^k1] Dönüşümlü düzen, aynı
topluluktan sürekli yeni kişi gelmesini sağlar.

Atlas bu yapıyı ayrıca kaydeder.[^k3] Dönüşümlü bir yükümlülük,
bireysel riski dağıtırken topluluk düzeyinde kaybı sürekli kılar.

## Nüfus üzerindeki etkisi

Düzen, nüfus hareketini de belirledi.[^k2] Yükümlülükten kaçmak için
topluluklarını terk edenler oldu.

Terk, yükümlülüğü kalanların üzerine yığdı.[^k1] Kota topluluk başına
belirlendiği için, azalan nüfus kişi başına yükü artırdı.

Atlas bu döngüyü ayrıca kaydeder.[^k3] Topluluk başına sabit bir
yükümlülük, nüfus azaldıkça kendiliğinden ağırlaşır.

## Kaydın tarafı

Düzenin yazılı kaydının büyük bölümü yönetim tarafından tutuldu.[^k1]
Kota listeleri, vergi kayıtları ve tahsis belgeleri aynı elden
çıkar.

Bir istisna vardır ve atlas bunu ayrıca künyeler.[^k2] Yerli bir
yazarın hazırladığı çizimli el yazması, düzeni tabi kılınan tarafın
konumundan anlatır.

Atlas iki kaydı yan yana kullanır.[^k3] Aynı uygulamanın iki kayıtta
nasıl adlandırıldığı, adlandırmanın kendisini bir veri hâline
getirir.

## Direnişin biçimleri

Direniş her zaman ayaklanma biçiminde olmadı.[^k1] Kaçma, hukuki
başvuru ve kota pazarlığı daha sık görülen biçimlerdir.

Hukuki başvuru şaşırtıcı ölçüde yaygındı.[^k2] Topluluklar, düzenin
kendi kurallarını kullanarak yükümlülüklerini azaltmaya çalıştı.

Atlas bu biçimi ayrıca kaydeder.[^k3] Bir düzenin kurallarını
kullanarak itiraz etmek, düzeni kabul etmek anlamına gelmez;
elde bulunan tek aracı kullanmak anlamına gelebilir.

## Diğer bölgelerdeki karşılıkları

Aynı yapı başka kıtalarda başka adlarla kuruldu.[^k1] Sözleşmeli işçi
taşıma, zorunlu ekim ve angarya vergisi bunlar arasındadır.

Ortak nokta üç ölçütte görünür.[^k3] Reddin mümkün olmaması,
satılabilir olmama ve karşılığın karşı tarafça belirlenmesi.

Atlas bu ortaklığı bir aile benzerliği sayar.[^k2] Aynı üç ölçütü
karşılayan düzenler, adları farklı olsa da aynı soruyla
incelenir.

## Sona ermesi

Düzenler farklı tarihlerde ve farklı biçimlerde sona erdi.[^k1] Bazıları
yasayla kaldırıldı, bazıları uygulanmaz hâle geldi.

Kaldırılma, emek sorununu ortadan kaldırmadı.[^k3] Yerine borç
bağımlılığı ve kiracılık gibi başka bağlama biçimleri geçti.

Atlas bu süreklilik sorusunu açık tutar.[^k2] Bir zorlama biçiminin
kaldırılması, zorlamanın sona erdiğini göstermez.

## Okuma yönlendirmesi

Düzenin dönemindeki en ayrıntılı eleştirisi için
[Bartolomé de las Casas](/dusunur/las-casas/) dosyası okunmalıdır.[^k1]

Tabi kılınan tarafın kaydı için
[Guamán Poma — Nueva Corónica](/kaynak/guaman-poma-nueva-coronica/),
düzene karşı en büyük ayaklanma için
[Túpac Amaru ayaklanması (1780)](/olay/tupac-amaru-ayaklanmasi-1780/)
dosyaları uygundur.[^k2]

Aynı yapının başka kıtadaki biçimi için
[Pasifik işçi toplama düzeni](/kavram/pasifik-isci-toplama/), tarım
düzenine etkisi için
[Tek ürüne bağlanma](/kavram/pamuk-tek-urun/) dosyaları okunabilir.[^k3]
