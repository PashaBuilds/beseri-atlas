---
id: kaynak-baburname
tip: kaynak
baslik: "Bâbür — Bâbürnâme"
ozet: >-
  Bir hükümdarın kendi yenilgilerini de yazdığı birinci ağızdan hatırat;
  atlasta bir yöneticinin kendi kararlarını gerekçelendirmek yerine
  anlattığı ender metin.
tarih_baslangic: "1530"
bolge: [orta-asya, guney-asya, islam-dunyasi]
eksen: [siyasi, kulturel, askeri]
guven_geneli: yaygin
etiketler: [baburname, babur, hatirat, oz-kayit, timurlu, hindistan]
ilgili:
  - aktor-babur-imparatorlugu
  - aktor-timur-imparatorlugu
  - kaynak-ain-i-akbari
  - kavram-mesruiyet
  - tartisma-buyuk-adam-kurami
  - aktor-harizmsahlar
okuma_onerisi: []
yazar: "Zahîrüddin Muhammed Bâbür"
eser_adi: "Bâbürnâme"
yayin_yili: 1530
telif_durumu: kamu-mali
tez_ozeti: >-
  Metin, bir hükümdarın gençliğinden ölümüne kadar geçen yılları kendi
  ağzından anlatır: kaybedilen şehirler, dağılan ordular, kurulan ittifaklar
  ve sonunda güneye yapılan sefer. Anlatının ayırt edici yanı, yenilgilerin
  ve kişisel zaafların da kaydedilmiş olmasıdır. Metin ayrıca gezdiği
  bölgelerin bitki örtüsünü, hayvanlarını, meyvelerini ve gündelik
  hayatını ayrıntılı biçimde tarif eder.
hangi_tartismada:
  - tartisma-buyuk-adam-kurami
kaynaklar:
  - anahtar: k1
    tur: birincil
    birincil_tur: eser
    ad: "The Bābur-nāma in English — A. S. Beveridge cevirisi, tam metin"
    url: https://www.gutenberg.org/ebooks/44608
    erisim_tarihi: 2026-08-25
    dogrulama_dizesi: "Babur"
    not: "Standart Ingilizce ceviri; kamu mali, makine okunur tam metin"
  - anahtar: k2
    tur: birincil
    birincil_tur: eser
    ad: "The Bābur-nāma in English (Memoirs of Bābur) — archive.org kopyasi"
    url: https://archive.org/details/thebburnmainengl44608gut
    erisim_tarihi: 2026-08-25
    dogrulama_dizesi: "Babur"
    not: "Ayni cevirinin ikinci erisim noktasi; bagimsiz alan adi"
  - anahtar: k3
    tur: birincil
    birincil_tur: belge
    ad: "Ain I Akbari — Abul Fazl, Blochmann cevirisi (1873)"
    url: https://archive.org/details/dli.pahar.0739
    erisim_tarihi: 2026-08-25
    dogrulama_dizesi: "Ain I Akbari"
    not: "Ayni hanedanin iki kusak sonraki idari kaydi; karsilastirma icin"
son_denetim: 2026-08-25
denetim_durumu: onaylandi
onarim_turu: 0
---

## Ne olduğu

Bâbürnâme, bir hükümdarın kendi hayatını birinci ağızdan anlattığı
hatırattır.[^k1][^k2] Anlatı gençlik yıllarındaki taht mücadelesiyle başlar
ve son seferlerle biter.

Metnin atlas açısından ayırt edici yanı, yenilgilerin de yazılmış
olmasıdır.[^k1] Kaybedilen şehirler, dağılan ordular ve yanlış hesaplanan
kararlar aynı ayrıntıyla kaydedilir.

Bu, hükümdar kayıtlarında olağan değildir.[^k3] Saray tarihlerinin çoğu
başarıyı ayrıntılandırır, başarısızlığı geçiştirir; burada ikisi aynı
kalemle yazılmıştır.

## Yenilgi kaydının değeri

Bir kaydın, yazarının aleyhine olan bilgiyi taşıması, kaydın
güvenilirliğini artıran bir işarettir.[^k1] Yazar, anlatısını çıkarına göre
düzenlememiş olur.

Bu ölçüt mutlak değildir. Bir yenilgiyi kabul etmek, sonraki başarıyı daha
etkileyici kılan bir anlatı tekniği de olabilir;[^k1] samimiyet ile
kurgu her zaman ayrılamaz.

Atlas bu yüzden metni bir doğruluk garantisi olarak değil, olağandışı bir
kayıt biçimi olarak künyeler.[^k2] Değeri, ne söylediği kadar nasıl
kurulduğundadır.

## Doğa ve gündelik hayat kayıtları

Metnin önemli bir bölümü, gezilen bölgelerin bitki örtüsüne, hayvanlarına,
meyvelerine ve yerel âdetlerine ayrılmıştır.[^k1] Bu bölümler siyasi
anlatının arasına serpiştirilmiştir.

Bu malzeme atlasın öteki kaynaklarında seyrek bulunur.[^k2] Bir hükümdarın
bir meyvenin tadını ya da bir kuşun göçünü kaydetmesi, dönemin çevre ve
gündelik hayat tarihi için doğrudan kanıt sağlar.

Kayıtların taraflılığı burada da vardır ve yönü farklıdır.[^k1] Yazar
kuzeyden gelmiştir ve güneydeki bölgeyi kendi memleketiyle karşılaştırarak
tarif eder; değerlendirmeleri bir kıyaslamadır, bir ölçüm değil.

## Karşılaştırmalı okuma

Bu dosya, atlasın Güney Asya kaynak katmanındaki idari envanterle birlikte
okunmak üzere yazılmıştır.[^k3] İki metin aynı hanedanın iki ayrı yüzünü
gösterir.

Fark keskindir. Hatırat bir kişinin kararlarını ve hatalarını anlatır;
envanter aynı hanedanın iki kuşak sonraki hesap defteridir ve kişiyi
görünmez kılar.[^k3]

İki metnin yan yana okunması, atlasın
[Büyük adam kuramı](/tartisma/buyuk-adam-kurami/) tartışmasına doğrudan
malzeme verir.[^k1] Aynı hanedan, bir metinde bir kişiye, ötekinde bir
sisteme benzer.

## Metnin durumu

Metin özgün dilinde yazılmış ve sonraki yüzyıllarda başka dillere
çevrilmiştir.[^k1][^k2] Kullanılan çeviri, standart kabul edilen ve kamu malı
olan baskıdır.

Metnin bazı bölümleri eksiktir ve elde bulunan nüshalarda boşluklar
vardır.[^k2] Bu boşluklar künyede gösterilmez ama metne atıf yapılırken
dikkate alınmalıdır.

Atlas metni iki ayrı alan adından künyeler.[^k1][^k2] Aynı çevirinin iki
erişim noktası bulunması, bir alan adının erişilemez hâle gelmesi durumunda
kaynağın kaybolmamasını sağlar.

## Kapsam dışı

Bu dosya metnin ne olduğunu ve nasıl okunması gerektiğini kaydeder; olayları
özetlemez.[^k1] Metin kaynaklarda erişilebilir durumdadır.

Yazarın kurduğu siyasi düzenin tarihi burada işlenmez.[^k3] Bunun için
[Babür İmparatorluğu](/aktor/babur-imparatorlugu/) dosyası okunmalıdır.

Metnin edebî özellikleri, dili ve sonraki edebiyat üzerindeki etkisi de
kapsam dışıdır.[^k2] Ayrı bir dosya gerektirir ve bu dosya yazıldığında
atlasta bulunmuyordu.

## Okuma yönlendirmesi

Yazarın kurduğu siyasi düzen için
[Babür İmparatorluğu](/aktor/babur-imparatorlugu/), geldiği hanedan çizgisi
için [Timur İmparatorluğu](/aktor/timur-imparatorlugu/) dosyaları
okunmalıdır.

Aynı hanedanın idari kaydı için
[Âyîn-i Ekberî](/kaynak/ain-i-akbari/), kişinin tarihteki payı sorusu için
[Büyük adam kuramı](/tartisma/buyuk-adam-kurami/) dosyaları uygundur.

Metne yapılacak atıflarda sayfa ve kullanılan çeviri belirtilmelidir; iki
künye aynı çeviriyi gösterir ve sayfa numaraları baskıya göre değişir.[^k1][^k2]
