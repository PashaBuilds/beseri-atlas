---
id: kaynak-braudel-akdeniz
tip: kaynak
baslik: "Braudel — Akdeniz"
ozet: >-
  Üç bölümlük eser; üçüncü bölüm olaylara, siyasete ve insanlara ayrılmıştır ve
  yazar bunu yüzeylerin zamanı sayar.
tarih_baslangic: "1949"
bolge: [avrupa, afrika, islam-dunyasi]
eksen: [ekonomik, kulturel, siyasi]
guven_geneli: yaygin
etiketler: [akdeniz, uc-bolum, zaman-katmani, cevre, olay]
ilgili:
  - dusunur-braudel
  - kavram-longue-duree
  - kavram-donemlendirme
okuma_onerisi: []
yazar: "Fernand Braudel"
eser_adi: "La Méditerranée"
yayin_yili: 1949
telif_durumu: telifli
tez_ozeti: >-
  Kaynak, eserin zamanı katmanlara ayırdığını yazar: ikinci düzeyde iktisadi,
  toplumsal ve kültürel tarih bulunur ve bu düzeyde değişim çevreninkinden çok
  daha hızlıdır; üçüncü düzey olayların zamanıdır ve yazar için yüzeylerin ve
  aldatıcı etkilerin zamanıdır. Eserin üçüncü bölümü olaylar, siyaset ve
  insanları işler.
hangi_tartismada:
  - tartisma-tarihsel-sayilar-nasil-okunur
kaynaklar:
  - anahtar: k1
    tur: ansiklopedi
    ad: "Wikipedia (EN) - Fernand Braudel"
    url: https://en.wikipedia.org/wiki/Fernand_Braudel
    erisim_tarihi: 2026-08-21
    dogrulama_dizesi: "Braudel"
  - anahtar: k2
    tur: akademik
    ad: "Stanford Encyclopedia of Philosophy - Karl Marx"
    url: https://plato.stanford.edu/entries/marx/
    erisim_tarihi: 2026-08-21
    dogrulama_dizesi: "Marx"
    not: "Ayrildigi yorumlardan biri; bagimsiz alan adi"
  - anahtar: k3
    tur: ansiklopedi
    ad: "Wikipedia (EN) - Periodization"
    url: https://en.wikipedia.org/wiki/Periodization
    erisim_tarihi: 2026-08-21
    dogrulama_dizesi: "Periodization"
    not: "Rakip donemlendirme onerileri"
son_denetim: 2026-08-21
denetim_durumu: onaylandi
onarim_turu: 0
---

## Künye

Kaynak eseri *La Méditerranée* adıyla anar ve ona ayrı bir bölüm ayırır;
yazarın 1902-1985 arasında yaşadığı kaydedilir.[^k1]

## Eser üç zaman katmanına göre bölünmüştür

Kaynak, ikinci düzeyde iktisadi, toplumsal ve kültürel tarihin bulunduğunu ve bu
düzeyde değişimin çevreninkinden çok daha hızlı olduğunu yazar; üçüncü düzeyin
olayların zamanı olduğunu, bunun adı olan bireylerin tarihi olduğunu ve yazar
için yüzeylerin ve aldatıcı etkilerin zamanı sayıldığını kaydeder.[^k1] Eserin
üçüncü bölümünün olaylar, siyaset ve insanları işlediği belirtilir.[^k1]

Bir eserin bölümlerinin değişim hızına göre sıralanması,[^k1] atlasın şu
kuralının dayanağıdır: bu dosyada eserden yapılan aktarımlar, hangi katmandan
geldiği belirtilerek verilir.

## Coğrafya toplumsal biçim üretir

Kaynak, eserin Akdeniz'inin denizin yanı sıra çöl ve dağlar olduğunu; çölün bütün
topluluğun taşındığı göçebe bir toplumsal örgütlenme biçimi ürettiğini, dağ
yaşamının ise yerleşik olduğunu yazar.[^k1]

Bir deniz tarihinin çölle ve dağla birlikte tanımlanması,[^k1] atlasın bölge
alanını neden çoğul tuttuğunun dayanağıdır.

## Kavram atlasın kendi katmanında

Eserin dayandığı zaman katmanı kavramı
[Longue durée](/kavram/longue-duree/) dosyasında, dönem bölmesinin genel sorunu
ise [Dönemlendirme](/kavram/donemlendirme/) dosyasında ele alınıyor; ikinci dosya
için tutulan kayıt üç çağ sistemine ayrı bir bölüm ayırır.[^k3]

## Ayrıldığı yorum geleneği

Kaynak, yazarın kapitalistlerin rekabetçi piyasalarda faaliyet gösteren
girişimciler değil tekelciler olduğunu savunduğunu ve bunun hem liberal hem
Marksçı yorumlardan ayrıldığını yazar.[^k1] Ayrıldığı yorumlardan biri için
tutulan kayıt, toplum biçimlerinin üretici gücün gelişimine göre yükselip düştüğü
fikrini anlatır.[^k2] O gelenek
[Karl Marx](/dusunur/marx/) dosyasında, yazarın kendisi ise
[Fernand Braudel](/dusunur/braudel/) dosyasında ele alınıyor.

## Okuma yönlendirmesi

Bu dosya eserin metnine bağlanmaz; kullanılan kaynak yazar hakkındaki bir
ansiklopedi maddesidir.[^k1] Eser teliflidir; atlas metninden alıntı yapmaz ve
yalnızca hakkında yazılanı aktarır.
