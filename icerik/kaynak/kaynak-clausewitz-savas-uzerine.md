---
id: kaynak-clausewitz-savas-uzerine
tip: kaynak
baslik: "Clausewitz — Savaş Üzerine"
ozet: >-
  Yazarın ölümünde tamamlanmamış olan eser; kaynağa göre metni dul eşi düzenledi,
  yayımladı ve önsözünü yazdı.
tarih_baslangic: "1832"
bolge: [avrupa]
eksen: [askeri, siyasi, felsefi]
guven_geneli: yaygin
etiketler: [savas-uzerine, tamamlanmamis, olum-sonrasi-yayin, yazarlik, prusya]
ilgili:
  - dusunur-clausewitz
  - kavram-surtunme
  - kavram-topyekun-savas
okuma_onerisi: []
yazar: "Carl von Clausewitz"
eser_adi: "Vom Kriege (On War)"
yayin_yili: 1832
telif_durumu: kamu-mali
tez_ozeti: >-
  Kaynak, eserin yazarın ölümünde tamamlanmamış olduğunu ve askerî strateji ile
  bilim üzerine temel bir inceleme sayıldığını yazar. Yazarın metin üzerinde
  1816'da çalışmaya başladığı ama tamamlamadığı; dul eşinin 1832'de eseri
  düzenlediği, yayımladığı ve giriş ile önsözünü yazdığı kaydedilir. Eserin
  vurgusu savaş yapmanın moral ve siyasi yönleridir.
hangi_tartismada:
  - tartisma-tarihsel-sayilar-nasil-okunur
kaynaklar:
  - anahtar: k1
    tur: ansiklopedi
    ad: "Wikipedia (EN) - Carl von Clausewitz"
    url: https://en.wikipedia.org/wiki/Carl_von_Clausewitz
    erisim_tarihi: 2026-08-21
    dogrulama_dizesi: "Clausewitz"
  - anahtar: k2
    tur: ansiklopedi
    ad: "Wikipedia (EN) - Fog of war"
    url: https://en.wikipedia.org/wiki/Fog_of_war
    erisim_tarihi: 2026-08-21
    dogrulama_dizesi: "Fog of war"
    not: "Eserden turetilen kavram"
  - anahtar: k3
    tur: akademik
    ad: "Stanford Encyclopedia of Philosophy - Niccolo Machiavelli"
    url: https://plato.stanford.edu/entries/machiavelli/
    erisim_tarihi: 2026-08-21
    dogrulama_dizesi: "Machiavelli"
    not: "Onceki yuzyilin savas ilkeleri yazari; bagimsiz alan adi"
  - anahtar: k4
    tur: birincil
    birincil_tur: eser
    ad: "On War (tam metin)"
    url: https://www.gutenberg.org/ebooks/1946
    erisim_tarihi: 2026-08-23
    dogrulama_dizesi: "On War"
    not: "Eserin kamu mali tam metni"
son_denetim: 2026-08-23
denetim_durumu: onaylandi
onarim_turu: 0
---

## Künye

Kaynak eseri *Vom Kriege* adıyla künyeler ve yazarın en dikkate değer eseri
olduğunu, ölümünde tamamlanmamış olmasına rağmen askerî strateji ve bilim üzerine
temel bir inceleme sayıldığını yazar.[^k1]

## Metni yazar tamamlamadı

Kaynak, dul eşinin savaş felsefesi üzerine olan başyapıtı 1832'de düzenlediğini,
yayımladığını ve giriş bölümünü yazdığını kaydeder; yazarın metin üzerinde
1816'da çalışmaya başladığı ama tamamlamadığı belirtilir.[^k1] Dul eşinin ayrıca
eserin önsözünü yazdığı ve 1835'e kadar toplu eserlerinin çoğunu yayımladığı
yazılır.[^k1]

Bir eserin bugünkü biçiminin yazarından sonra bir başkasının düzenleme
kararlarıyla oluşması,[^k1] korpusun şu kuralının dayanağıdır: bu incelemede esere
atfedilen hiçbir önerme, hangi düzenlemeye ait olduğu belirtilmeden tek bir
yazara mal edilmez.

## Yazar ölümünden önceki görevdeydi

Kaynak, yazarın Prusya'nın seferber edebildiği tek ordunun kurmay başkanı olarak
atandığını, büyük kolera salgınını sınırlamak için kurulan karantina hattının
komutasını üstlendiğini ve kısa süre sonra aynı hastalıktan 16 Kasım 1831'de
öldüğünü yazar.[^k1]

Eserin yayım yılı ile yazarın ölüm yılı arasındaki bir yıllık fark,[^k1] künye
alanında yayın yılının 1832 olarak tutulmasının gerekçesidir.

## Eserden türetilen kavram

Eserden türetilen bir kavram için tutulan ayrı kayıt, kavramın köken, askerî
kullanım ve benzetimler ile oyunlar bölümlerini tutar.[^k2] O kavram
[Sürtünme](/kavram/surtunme/) dosyasında ele alınıyor.

## Önceki yüzyılın savaş yazarı

Önceki yüzyılın savaş ilkeleri yazarı için tutulan kayıt, katkı alanları arasında
savaş ilkelerini sayar.[^k3] O yazar
[Machiavelli](/dusunur/machiavelli/) dosyasında, yazarın kendisi ise
[Carl von Clausewitz](/dusunur/clausewitz/) dosyasında ele alınıyor.

## Metne bağ

Bu inceleme eserin kamu malı tam metnine bağlanır.[^k4] Eserin kendisi kamu malıdır; hangi baskının ve
hangi çevirinin kullanıldığı her aktarımda belirtilmelidir.
