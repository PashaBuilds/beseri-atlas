---
id: kaynak-hegel-gorungubilim
tip: kaynak
baslik: "Hegel — Tinin Görüngübilimi"
ozet: >-
  1806 sonunda tamamlanan, 1807'de yayımlanan eser; kaynağa göre önsözündeki
  iğneli bir eleştiri bir dostluğu bitirdi.
tarih_baslangic: "1807"
bolge: [avrupa]
eksen: [felsefi]
guven_geneli: tartismali
etiketler: [gorungubilim, onsoz, schelling, yorum-ayrimi, jena]
ilgili:
  - dusunur-hegel
  - kaynak-kant-elestiri
  - dusunur-marx
okuma_onerisi: []
yazar: "G. W. F. Hegel"
eser_adi: "Phänomenologie des Geistes"
yayin_yili: 1807
telif_durumu: kamu-mali
tez_ozeti: >-
  Kaynak, yazarın 1806 sonunda ilk büyük eserini tamamladığını (1807'de
  yayımlandı) ve eserin daha önceki, görünüşte daha Schellingci yaklaşımından bir
  ayrışma gösterdiğini yazar. Aynı kayıt eserin yazımı sırasında Napolyon'un
  birliklerinin Jena'yı işgalinin üniversitenin faaliyetlerini kısıtladığını da
  belirtir.
hangi_tartismada:
  - tartisma-tarihsel-sayilar-nasil-okunur
kaynaklar:
  - anahtar: k1
    tur: akademik
    ad: "Stanford Encyclopedia of Philosophy - Georg Wilhelm Friedrich Hegel"
    url: https://plato.stanford.edu/entries/hegel/
    erisim_tarihi: 2026-08-21
    dogrulama_dizesi: "Hegel"
  - anahtar: k2
    tur: ansiklopedi
    ad: "Wikipedia (EN) - Napoleonic Wars"
    url: https://en.wikipedia.org/wiki/Napoleonic_Wars
    erisim_tarihi: 2026-08-21
    dogrulama_dizesi: "Napoleonic Wars"
    not: "Yazim kosullarini belirleyen savaslar; bagimsiz alan adi"
  - anahtar: k3
    tur: akademik
    ad: "Stanford Encyclopedia of Philosophy - Immanuel Kant"
    url: https://plato.stanford.edu/entries/kant/
    erisim_tarihi: 2026-08-21
    dogrulama_dizesi: "Kant"
    not: "Okumalardan birinin adini aldigi dusunur"
  - anahtar: k4
    tur: birincil
    birincil_tur: eser
    ad: "The phenomenology of mind (tam metin)"
    url: https://archive.org/details/phenomenologyofm0000hege
    erisim_tarihi: 2026-08-23
    dogrulama_dizesi: "The phenomenology of mind"
    not: "Eserin kamu mali tam metni; Hegel, Georg Wilhelm Friedrich (1770-1831) kunyeli 1949 baskisi"
son_denetim: 2026-08-23
denetim_durumu: onaylandi
onarim_turu: 0
---

## Künye

Kaynak, yazarın 1806 sonunda ilk büyük eserini tamamladığını ve eserin 1807'de
yayımlandığını yazar; eserin daha önceki, görünüşte daha Schellingci
yaklaşımından bir ayrışma gösterdiği kaydedilir.[^k1]

## Önsöz bir dostluğu bitirir

Kaynak, Schelling'in eserin önsözündeki iğneli bir eleştiriyi kendisine yönelik
yorumladığını ve dostluklarının aniden sona erdiğini yazar.[^k1]

Bir metnin önsözünün bir ilişkiyi bitirmesi,[^k1] korpusun şu kuralının
dayanağıdır: bu esere yapılan atıflar, önsöz ile ana metnin ayrı ayrı
belirtilmesini gerektirir.

## Yazım koşulları kayıtlıdır

Kaynak, elyazmasını tamamlarken Napolyon'un birliklerinin Jena'yı işgalinin
üniversitenin faaliyetlerini kısıtladığını ve yazarın oradan ayrıldığını
yazar.[^k1] İlgili savaşlar
[Napolyon Savaşları](/olay/napolyon-savaslari/) dosyasında ele alınıyor.[^k2]

## Eserin okuması tartışmalıdır

::tartismali[Kaynak yazarın felsefesine dair üç ayrı okumayı alt başlıklar hâlinde sıralıyor ve hangisinin doğru olduğunu söylemiyor. Korpus da söylemez.]{harita=tartisma-tarihsel-sayilar-nasil-okunur}

Kaynak okumaları şöyle adlandırır: geleneksel metafizik görüş, kimi zaman
metafizik olmayan görüş de denen Kant sonrası görüş ve Kant sonrası görüşe karşı
gözden geçirilmiş metafizik tepki.[^k1] Okumalardan birinin adını aldığı düşünür
[Immanuel Kant](/dusunur/kant/) dosyasında, eseri ise
[Kant — Saf Aklın Eleştirisi](/kaynak/kant-elestiri/) dosyasında ele
alınıyor.[^k3] Yazar
[G. W. F. Hegel](/dusunur/hegel/) dosyasında ele alınıyor.

## Okumayı sürdürmek için
Bu inceleme eserin metnine bağlanmaz; kullanılan kaynak yazar hakkındaki bir
ansiklopedi maddesidir.[^k1] Eser kamu malıdır; hangi baskının ve hangi çevirinin
kullanıldığı her aktarımda belirtilmelidir.

## Metne bağ

Bu inceleme eserin kamu malı tam metnine bağlanır.[^k4] Esere yapılacak her
atıf bölüm ya da paragraf numarasıyla verilmeli; hangi baskının ve hangi
çevirinin kullanıldığı her aktarımda belirtilmelidir.
