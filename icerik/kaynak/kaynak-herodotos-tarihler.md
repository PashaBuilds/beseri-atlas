---
id: kaynak-herodotos-tarihler
tip: kaynak
baslik: "Herodotos — Tarihler"
ozet: >-
  Yunan-Pers Savaşları'nın ayrıntılı anlatısı; kaynak eserin hatırı sayılır bir
  bölümünün sonradan doğrulandığını da, çağdaş suçlamayı da kaydeder.
tarih_baslangic: "-0430"
bolge: [avrupa, islam-dunyasi, afrika]
eksen: [kulturel, siyasi, askeri]
guven_geneli: tartismali
etiketler: [tarihler, yunan-pers, taniklik, dogrulama, elestiri]
ilgili:
  - dusunur-herodotos
  - olay-pers-yunan-savaslari
  - kavram-kaynak-elestirisi
  - tartisma-tarihsel-sayilar-nasil-okunur
okuma_onerisi: []
yazar: "Herodotos"
eser_adi: "Historíai (Tarihler)"
yayin_yili: -430
telif_durumu: kamu-mali
tez_ozeti: >-
  Kaynak eseri Yunan-Pers Savaşları'nın ayrıntılı bir anlatısı olarak tanımlar ve
  başka konular arasında Kyros'un Ahameniş hanedanının yükselişini sayar. Aynı
  kayıt, yazarın kendi zamanında eserine efsaneler ve hayalî anlatılar kattığı
  için eleştirildiğini, buna karşı gördüğünü ve kendisine anlatılanı bildirdiğini
  söyleyerek karşılık verdiğini ve eserin hatırı sayılır bir bölümünün o zamandan
  bu yana modern tarihçiler ve arkeologlarca doğrulandığını yazar.
hangi_tartismada:
  - tartisma-tarihsel-sayilar-nasil-okunur
kaynaklar:
  - anahtar: k1
    tur: ansiklopedi
    ad: "Wikipedia (EN) - Herodotus"
    url: https://en.wikipedia.org/wiki/Herodotus
    erisim_tarihi: 2026-08-21
    dogrulama_dizesi: "Herodotus"
  - anahtar: k2
    tur: ansiklopedi
    ad: "1911 Encyclopaedia Britannica - Herodotus (Wikisource)"
    url: https://en.wikisource.org/wiki/1911_Encyclop%C3%A6dia_Britannica/Herodotus
    erisim_tarihi: 2026-08-21
    dogrulama_dizesi: "HERODOTUS"
    not: "Yuz yil onceki kayit; bagimsiz alan adi"
  - anahtar: k3
    tur: ansiklopedi
    ad: "Wikipedia (EN) - History of the Peloponnesian War"
    url: https://en.wikipedia.org/wiki/History_of_the_Peloponnesian_War
    erisim_tarihi: 2026-08-21
    dogrulama_dizesi: "History of the Peloponnesian War"
    not: "Kaynagin yontem bakimindan karsilastirdigi eser"
son_denetim: 2026-08-21
denetim_durumu: onaylandi
onarim_turu: 0
---
## Künye

Kaynak eseri *Tarihler* adıyla anar ve Yunan-Pers Savaşları'nın ayrıntılı bir
anlatısı olarak tanımlar; başka konular arasında Kyros'un Ahameniş hanedanının
yükselişini sayar.[^k1]

## Eserin statüsü aynı maddede iki yönlüdür

::tartismali[Kaynak hem yazarın çağdaşı tarafından hikâye uydurmakla suçlandığını hem eserin hatırı sayılır bir bölümünün sonradan doğrulandığını yazıyor.]{harita=tartisma-tarihsel-sayilar-nasil-okunur}

Kaynak, yazarın kendi zamanında eserine efsaneler ve hayalî anlatılar kattığı
için eleştirildiğini ve Peloponez Savaşı'nı yazan çağdaşının onu eğlence olsun
diye hikâye uydurmakla suçladığını kaydeder; aynı madde eserin hatırı sayılır bir
bölümünün o zamandan bu yana modern tarihçiler ve arkeologlar tarafından
doğrulandığını da yazar.[^k1] Okuma yöntemi
[Tarihsel sayılar nasıl okunur?](/tartisma/tarihsel-sayilar-nasil-okunur/)
haritasında ele alınıyor.

## Yazarın kendi yöntem beyanı

Kaynak, yazarın bu suçlamaya gördüğünü ve kendisine anlatılanı bildirdiğini
söyleyerek karşılık verdiğini yazar.[^k1]

## Yöntem bakımından karşılaştırılan eser

Kaynak, yazarın kronoloji yönteminin bir başka eserinkinden keskin biçimde
ayrıldığını yazar; o eser için tutulan ayrı kayıt, yazarının olayları yıl yıl
kaydettiğini ve her yılın bir yaz sefer mevsimi ile daha az hareketli bir kış
mevsiminden oluştuğunu belirtir.[^k3] O eser
[Thukydides — Peloponez Savaşı'nın Tarihi](/kaynak/thukydides-peloponnesos/)
dosyasında ele alınıyor.

## Yüz yıl önceki kayıt

Yüz yıl önce yazılmış bir başka ansiklopedi maddesi, yazarı tarihin babası diye
anılan Yunan tarihçi olarak tanıtır ve Halikarnassos'ta doğduğunu yazar.[^k2]
