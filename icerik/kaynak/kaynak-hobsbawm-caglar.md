---
id: kaynak-hobsbawm-caglar
tip: kaynak
baslik: "Hobsbawm — Çağlar dizisi"
ozet: >-
  1962'de başlayan dört ciltlik dizi; atlasın 11 ve 12 numaralı dönem adları bu
  dizinin kitap adlarıyla örtüşür ve bu örtüşme burada açıkça kaydedilir.
tarih_baslangic: "1962"
tarih_bitis: "1994"
bolge: [avrupa, kuresel]
eksen: [siyasi, ekonomik, kulturel]
guven_geneli: tartismali
etiketler: [donemlendirme, ikiz-devrim, kisa-yuzyil, kitap-dizisi, ad-verme]
ilgili:
  - kavram-donemlendirme
  - donem-11
  - donem-12
  - donem-13
  - tartisma-vestfalya-miti
okuma_onerisi: []
yazar: "Eric Hobsbawm"
eser_adi: "The Age of Revolution: Europe 1789–1848"
yayin_yili: 1962
telif_durumu: telifli
tez_ozeti: >-
  Kaynak, kitabı uzun 19. yüzyıl üzerine bir üçlemenin ilki olarak kaydeder;
  devamında The Age of Capital: 1848-1875 ve The Age of Empire: 1875-1914
  gelir. Yazarın çözümlemesi, Fransız Devrimi ile Sanayi Devrimi'nin modern
  Avrupa tarihinin birlikte doğurtucusu sayıldığı ikiz devrim tezine dayanır.
  Dizinin 1994 tarihli dördüncü kitabı, Birinci Dünya Savaşı'nın başından Sovyet
  blokunun çöküşüne uzanan aralığı kısa yirminci yüzyıl olarak adlandırır.
hangi_tartismada:
  - tartisma-vestfalya-miti
kaynaklar:
  - anahtar: k1
    tur: ansiklopedi
    ad: "Wikipedia (EN) - The Age of Revolution: Europe 1789-1848"
    url: https://en.wikipedia.org/wiki/The_Age_of_Revolution:_Europe_1789%E2%80%931848
    erisim_tarihi: 2026-08-21
    dogrulama_dizesi: "The Age of Revolution"
    not: "Kitabin kendi sayfasi; The_Age_of_Revolution adresi doneme yonlendiriyor"
  - anahtar: k2
    tur: ansiklopedi
    ad: "Wikipedia (EN) - The Age of Extremes"
    url: https://en.wikipedia.org/wiki/The_Age_of_Extremes
    erisim_tarihi: 2026-08-21
    dogrulama_dizesi: "Age of Extremes"
    not: "Dizinin 20. yuzyil cildi"
  - anahtar: k3
    tur: veri
    ad: "Our World in Data - Population, long-run with projections (CSV verisi)"
    url: https://ourworldindata.org/grapher/population-long-run-with-projections.csv?csvType=full&useColumnShortNames=true
    erisim_tarihi: 2026-08-21
    dogrulama_dizesi: "population_historical"
    not: "Bagimsiz alan adi (Ilke 6); donem sinirlarinin olculebilir capasi"
son_denetim: 2026-08-21
denetim_durumu: onaylandi
onarim_turu: 0
---

Bu dosya bir kitabı değil, bir **ad verme kararını** ele alır: atlasın kendi
dönem adlarının nereden geldiğini.

## Dizinin ilk kitabı

Kaynak, kitabı İngiliz tarihçi Eric Hobsbawm'ın 1962'de ilk kez yayımlanan eseri
olarak kaydeder; 366 sayfadır ve Birleşik Krallık'ta Weidenfeld & Nicolson,
Amerika Birleşik Devletleri'nde World Publishing tarafından basılmıştır.[^k1]

Kaynak, kitabın çözümlemesini ikiz devrim tezine dayandırır: Fransız Devrimi ile
Sanayi Devrimi, modern Avrupa tarihinin birlikte doğurtucusu sayılır.[^k1]

İki ayrı olayın tek bir tezde birleştirilmesi,[^k1] bir dönem adının bir
betimleme değil bir sınıflandırma kararı olduğunu gösterir; Sanayi Devrimi
[Sanayi Devrimi](/olay/sanayi-devrimi/) dosyasında ele alınıyor.

## Üçleme dönemleri adlandırır

Kaynak, kitabı uzun 19. yüzyıl üzerine bir üçlemenin ilki sayar; devamındaki
ciltler The Age of Capital: 1848-1875 ve The Age of Empire: 1875-1914'tür.[^k1]

::tartismali[Atlasın 11 ve 12 numaralı dönem adları bu kitap adlarıyla örtüşür. Bu bir doğrulama değil, bir miras alma durumudur ve burada kaydedilmiştir.]{harita=tartisma-vestfalya-miti}

Kaynaktaki üç kitap adı sırasıyla 1789-1848, 1848-1875 ve 1875-1914
aralıklarını taşır;[^k1] atlasın dönem listesindeki Devrim Çağı ile Sermaye ve
İmparatorluk Çağı adları bu adların karşılığı ve birleşimidir. Dönemler
[1789–1848](/donem/11/) ve [1848–1914](/donem/12/) dosyalarında ele alınıyor.

## Yirminci yüzyıl cildi

Dizinin dördüncü kitabı için tutulan kayıt, onu 1994'te yayımlanan 640 sayfalık
bir eser olarak verir; kendisinden önce The Age of Empire: 1875-1914
gelmektedir.[^k2]

Kaynak, yazarın Birinci Dünya Savaşı'nın başlangıcından Sovyet blokunun
çöküşüne uzanan dönemi kısa yirminci yüzyıl diye adlandırdığını ve bunun uzun
19. yüzyılın devamı sayıldığını yazar; uzun 19. yüzyıl 1789'daki Fransız
Devrimi'nin başlangıcından 1914'e uzanır.[^k2]

Bir yüzyılın takvim yüzyılından farklı sınırlarla tanımlanması,[^k2] atlasın şu
kuralının dayanağıdır: dönem sınırları takvimden değil tezden gelir ve hangi
tezden geldiği yazılmalıdır. Atlasın 1914-1945 dönemi
[1914–1945](/donem/13/) dosyasında, dönemlendirmenin kendisi ise
[Dönemlendirme](/kavram/donemlendirme/) dosyasında ele alınıyor.

## Aynı kitabın iki alt başlığı

Kaynak, dördüncü kitabın Amerika Birleşik Devletleri baskısının farklı bir alt
başlıkla çıktığını kaydeder: A History of the World, 1914-1991.[^k2]

Aynı kitabın iki ülkede iki farklı alt başlıkla yayımlanması,[^k2] bir dönem
adının yazarın tezi kadar yayıncının kararına da bağlı olabildiğini gösterir.

## Sınırların ölçülebilir çapası

Uzun dönemli seride dünya nüfusu 1790'da yaklaşık 942,3 milyon, 1848'de yaklaşık
1,27 milyar, 1914'te yaklaşık 1,82 milyar ve 1991'de yaklaşık 5,42 milyardır.[^k3]

Bu dört değer bir dönemlendirmeyi doğrulamaz; yalnızca 1789-1991 aralığında
dünya nüfusunun yaklaşık beş buçuk kat arttığını gösterir.[^k3]

## Telif sınırı

Kitaplar telifli olduğu için bu dosyada metinlerinden alıntı yapılmamıştır; 1962
ve 1994 tarihli iki cildin içeriğine dair buradaki bütün ifadeler kaynak
sayfalarının kendi anlatımına dayanır.[^k1][^k2]

## Bu dosyanın sınırı

Kitapların bölüm yapısı, aldığı eleştiriler ve yazarın diğer eserleri burada ele
alınmamıştır; bu dosyanın kapsamı, atlasın dönem adlarının kaynağını
kaydetmektir.
