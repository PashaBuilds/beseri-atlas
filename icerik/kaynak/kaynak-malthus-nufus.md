---
id: kaynak-malthus-nufus
tip: kaynak
baslik: "Malthus — Nüfus İlkesi Üzerine"
ozet: >-
  Kaynak eserin baskıları ve sürümleri için ayrı bir alt başlık tutar; birinci
  baskıda bulunan bir bölüm sonrakilerde anılmaz.
tarih_baslangic: "1798"
bolge: [avrupa, kuresel]
eksen: [demografik, ekonomik]
guven_geneli: tartismali
etiketler: [nufus-ilkesi, baski-farki, ahlaki-kisitlama, zihin-kurami, model]
ilgili:
  - dusunur-malthus
  - kavram-malthus-tuzagi
  - veri-nufus-artis-hizi
okuma_onerisi: []
yazar: "Thomas Robert Malthus"
eser_adi: "An Essay on the Principle of Population"
yayin_yili: 1798
telif_durumu: kamu-mali
tez_ozeti: >-
  Kaynak eser için ayrı bir bölüm açar ve altında baskılar ile sürümler diye bir
  alt başlık tutar. İkinci ve sonraki baskılarda alt sınıfların yoksulluğunu
  hafifletmenin en iyi yolu olarak ahlaki kısıtlamaya daha fazla vurgu yapıldığı;
  birinci baskının ayrıca insan davranışını açıklamaya çalışan gelişkin bir zihin
  kuramı içerdiği kaydedilir.
hangi_tartismada:
  - tartisma-tarihsel-sayilar-nasil-okunur
kaynaklar:
  - anahtar: k1
    tur: ansiklopedi
    ad: "Wikipedia (EN) - Thomas Robert Malthus"
    url: https://en.wikipedia.org/wiki/Thomas_Robert_Malthus
    erisim_tarihi: 2026-08-21
    dogrulama_dizesi: "Malthus"
  - anahtar: k2
    tur: ansiklopedi
    ad: "Wikipedia (EN) - Malthusianism"
    url: https://en.wikipedia.org/wiki/Malthusianism
    erisim_tarihi: 2026-08-21
    dogrulama_dizesi: "Malthusianism"
    not: "Eserden turetilen kavram"
  - anahtar: k3
    tur: veri
    ad: "Our World in Data - Population growth rates (CSV verisi)"
    url: https://ourworldindata.org/grapher/population-growth-rates.csv?csvType=full&useColumnShortNames=true
    erisim_tarihi: 2026-08-21
    dogrulama_dizesi: "growth_rate"
    not: "Modelin sinandigi seri; bagimsiz alan adi"
  - anahtar: k4
    tur: birincil
    birincil_tur: eser
    ad: "An Essay on the Principle of Population (tam metin)"
    url: https://www.gutenberg.org/ebooks/4239
    erisim_tarihi: 2026-08-23
    dogrulama_dizesi: "An Essay on the Principle of Population"
    not: "Eserin kamu mali tam metni"
son_denetim: 2026-08-23
denetim_durumu: onaylandi
onarim_turu: 0
---

## Künye

Kaynak eser için ayrı bir bölüm açar ve altında baskılar ile sürümler diye bir
alt başlık tutar; yazarı 1766-1834 arasına yerleştirir.[^k1]

## Birinci baskı sonrakilerden farklıdır

::tartismali[Kaynak, birinci baskıda bulunan bir bölümün sonraki baskılarda ayrıca anılmadığını kaydediyor. "Malthus şöyle der" cümlesi hangi baskıya dayandığı belirtilmeden kurulamaz.]{harita=tartisma-tarihsel-sayilar-nasil-okunur}

Kaynak, ikinci ve sonraki baskılarda alt sınıfların yoksulluğunu hafifletmenin en
iyi yolu olarak ahlaki kısıtlamaya daha fazla vurgu yapıldığını; birinci baskının
ayrıca insan davranışını açıklamaya çalışan gelişkin bir zihin kuramı içerdiğini
ve bu maddeci zihin anlayışının zihnin bedensel ve akıldışı doğasını öne
çıkardığını yazar.[^k1] Okuma yöntemi
[Tarihsel sayılar nasıl okunur?](/tartisma/tarihsel-sayilar-nasil-okunur/)
haritasında ele alınıyor.

## Modelin mekanizması kayıtlıdır

Kaynak, nüfus ile iktisat arasındaki ilişki hakkında şunu yazdığını kaydeder:
emekçi nüfusu gıda üretiminden hızlı büyüdüğünde gerçek ücretler düşer; aile
geçindirmenin güçlükleri sonunda nüfus artış hızını azaltır ve düşen nüfus
yeniden daha yüksek gerçek ücretlere yol açar.[^k1]

Kavram [Malthus
tuzağı](/kavram/malthus-tuzagi/) dosyasında ele alınıyor.[^k2][^k1]

## Modelin sınandığı seri

Nüfus artış hızı serisinde sağlayıcının toplam satırı 1950 için yüzde 1,738,
2023 için yüzde 0,871 verir.[^k3]

Seri [Nüfus artış hızı,
1950–2023](/veri/nufus-artis-hizi/) dosyasında, yazar ise [Thomas
Malthus](/dusunur/malthus/) dosyasında ele alınıyor.[^k1][^k3]

## Okumayı sürdürmek için
Bu inceleme eserin metnine bağlanmaz; kullanılan kaynak yazar hakkındaki bir
ansiklopedi maddesidir.[^k1] Eser kamu malıdır; hangi baskının kullanıldığı her
aktarımda belirtilmelidir.

## Metne bağ

Bu inceleme eserin kamu malı tam metnine bağlanır.[^k4] Esere yapılacak her
atıf bölüm ya da paragraf numarasıyla verilmeli; hangi baskının ve hangi
çevirinin kullanıldığı her aktarımda belirtilmelidir.
