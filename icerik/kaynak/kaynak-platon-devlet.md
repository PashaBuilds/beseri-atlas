---
id: kaynak-platon-devlet
tip: kaynak
baslik: "Platon — Devlet"
ozet: >-
  Kaynak eseri kitap kitap, hatta konuşmacı konuşmacı bölümler ve her bölüme
  metindeki satır aralığını verir.
tarih_baslangic: "-0375"
bolge: [avrupa]
eksen: [siyasi, felsefi]
guven_geneli: yaygin
etiketler: [devlet, kitap-yapisi, satir-araligi, konusmaci, adalet]
ilgili:
  - dusunur-platon
  - kavram-mesruiyet
  - kavram-sehir-devleti
okuma_onerisi: []
yazar: "Platon"
eser_adi: "Politeia (Devlet)"
yayin_yili: -375
telif_durumu: kamu-mali
tez_ozeti: >-
  Kaynak eseri kitaplara ayırır ve her kitaba metindeki satır aralığını verir:
  birinci kitap yaşlanma, sevgi ve adaletin tanımları (327a-354c), ikinci kitap
  Gyges'in yüzüğü (357a-383c), üçüncü kitap soylu yalan (386a-417b). Birinci
  kitabın altında Kephalos, Polemarkhos ve Thrasymakhos için ayrı alt başlıklar
  tutulur.
hangi_tartismada:
  - tartisma-tarihsel-sayilar-nasil-okunur
kaynaklar:
  - anahtar: k1
    tur: ansiklopedi
    ad: "Wikipedia (EN) - Republic (Plato)"
    url: https://en.wikipedia.org/wiki/Republic_(Plato)
    erisim_tarihi: 2026-08-21
    dogrulama_dizesi: "Republic"
  - anahtar: k2
    tur: akademik
    ad: "Stanford Encyclopedia of Philosophy - Plato"
    url: https://plato.stanford.edu/entries/plato/
    erisim_tarihi: 2026-08-21
    dogrulama_dizesi: "Plato"
    not: "Yazarin kendi maddesi; bagimsiz alan adi"
  - anahtar: k4
    tur: birincil
    birincil_tur: eser
    ad: "The Republic of Plato (tam metin)"
    url: https://www.gutenberg.org/ebooks/55201
    erisim_tarihi: 2026-08-23
    dogrulama_dizesi: "The Republic of Plato"
    not: "Eserin kamu mali tam metni"
son_denetim: 2026-08-23
denetim_durumu: onaylandi
onarim_turu: 0
---
## Künye

Kaynak eseri *Devlet* adıyla anar ve ilk bölümünde eserin yazarın külliyatındaki
yerini ayrıca işler.[^k1]

## Bölümleme satır aralıklarıyla verilir

Kaynak kitapları konu ve satır aralığıyla birlikte sıralar: birinci kitap
yaşlanma, sevgi ve adaletin tanımları (327a-354c), ikinci kitap Gyges'in yüzüğü
(357a-383c), üçüncü kitap soylu yalan (386a-417b).[^k1]

## Konuşmacılar ayrı alt başlıklardır

Kaynak, birinci kitabın altında Kephalos (328e-331d), Polemarkhos (331d-336b) ve
Thrasymakhos (336b-354c) için ayrı alt başlıklar tutar.[^k1]

Bir metnin konuşmacılarına ayrı başlık ayrılması,[^k1] yazarın kendi görüşünün
metinde doğrudan yer almadığı sorununu görünür kılar; o sorun
[Platon](/dusunur/platon/) dosyasında ele alınıyor.[^k2]

## Öğrencisinin aynı konudaki eseri

Öğrencisinin aynı konudaki eseri için tutulan kayıt, o eserin kitap kitap
bölümlendiğini ve anayasaların sınıflandırılmasına ayrı bir bölüm ayrıldığını
yazar. O eser
[Aristoteles — Politika](/kaynak/aristoteles-politika/) dosyasında ele alınıyor.

## Metne bağ

Eserin kamu malı tam metni erişilebilir durumdadır; hangi çevirinin kullanıldığı ve
hangi satır aralığına atıf yapıldığı her aktarımda belirtilmelidir.[^k4]
