---
id: kaynak-polanyi-buyuk-donusum
tip: kaynak
baslik: "Polanyi — Büyük Dönüşüm"
ozet: >-
  Kaynak eseri tarihsel toplumbilim için bir model sayar; aynı kayıt yaklaşımın
  eski toplumlardaki genel yararının sorgulandığını da yazar.
tarih_baslangic: "1944"
bolge: [avrupa, amerika]
eksen: [ekonomik, kulturel, siyasi]
guven_geneli: tartismali
etiketler: [buyuk-donusum, gomululuk, cift-hareket, model, itiraz]
ilgili:
  - dusunur-polanyi
  - kavram-kapitalizm
  - tartisma-bati-neden-yukseldi
okuma_onerisi: []
yazar: "Karl Polanyi"
eser_adi: "The Great Transformation"
yayin_yili: 1944
telif_durumu: telifli
tez_ozeti: >-
  Kaynak, eserin tarihsel toplumbilim için bir model hâline geldiğini yazar ve
  yazarın çift hareket kavramını piyasalaşma ile ona karşı toplumsal korunma
  talebinin diyalektik süreci olarak tanımladığını kaydeder; yazarın modern
  Avrupa'daki piyasa temelli toplumların kaçınılmaz değil tarihsel olarak
  olumsal olduğunu savunduğu belirtilir.
hangi_tartismada:
  - tartisma-bati-neden-yukseldi
kaynaklar:
  - anahtar: k1
    tur: ansiklopedi
    ad: "Wikipedia (EN) - Karl Polanyi"
    url: https://en.wikipedia.org/wiki/Karl_Polanyi
    erisim_tarihi: 2026-08-21
    dogrulama_dizesi: "Polanyi"
  - anahtar: k2
    tur: akademik
    ad: "Stanford Encyclopedia of Philosophy - Adam Smith's Moral and Political Philosophy"
    url: https://plato.stanford.edu/entries/smith-moral-political/
    erisim_tarihi: 2026-08-21
    dogrulama_dizesi: "Adam Smith"
    not: "Etki listesinde sayilan yazar; bagimsiz alan adi"
  - anahtar: k3
    tur: ansiklopedi
    ad: "Wikipedia (EN) - Fernand Braudel"
    url: https://en.wikipedia.org/wiki/Fernand_Braudel
    erisim_tarihi: 2026-08-21
    dogrulama_dizesi: "Braudel"
    not: "Ayni yillarda iktisadi tarih yazan yazar"
son_denetim: 2026-08-21
denetim_durumu: onaylandi
onarim_turu: 0
---

## Künye

Kaynak eseri *The Great Transformation* adıyla anar ve onun tarihsel toplumbilim
için bir model hâline geldiğini yazar; yazarın disiplinlerini iktisat
toplumbilimi, iktisat tarihi, iktisadi antropoloji ve felsefe olarak
sıralar.[^k1]

## Kaynak, eserin itirazını da kaydeder

Kaynak, yazarın eski iktisatlara yaklaşımının Kolomb öncesi Amerika ve eski
Mezopotamya gibi çeşitli örneklere uygulandığını, ancak eski toplumların
incelenmesinde genel yararının sorgulandığını yazar.[^k1]

::tartismali[Aynı paragraf hem eserin bir model sayıldığını hem yaklaşımın genel yararının sorgulandığını taşıyor. Korpus ikisini de kaydeder ve hangisinin ağır bastığını söylemez.]{harita=tartisma-bati-neden-yukseldi}

Bu tür yan yana kayıtların nasıl okunacağı
[Batı neden yükseldi?](/tartisma/bati-neden-yukseldi/) haritasında ele
alınıyor.

## Kavramlar ad ad sayılır

Kaynak, yazarın kayda değer fikirleri arasında gömülülük, çift hareket, kurgusal
metalar, iktisatçı yanılgısı ve biçimci-özcü tartışmasını sayar; çift hareket
kavramını piyasalaşma ile ona karşı toplumsal korunma talebinin diyalektik
süreci olarak tanımlar.[^k1]

Bir eserin katkısının kavram listesiyle kaydedilmesi,[^k1] korpusun kavram
katmanının neden ayrı bir tip olarak kurulduğunun dayanağıdır; ilgili düzen
[Kapitalizm](/kavram/kapitalizm/) dosyasında ele alınıyor.

## Kaçınılmazlık reddi kayıtlıdır

Kaynak, yazarın modern Avrupa'daki piyasa temelli toplumların kaçınılmaz değil
tarihsel olarak olumsal olduğunu savunduğunu yazar.[^k1]

Bir kuramın kendi konusunun kaçınılmaz olmadığını öne sürmesi,[^k1] korpusun
kaçınılmazlık cümlelerine neden temkinle yaklaştığıyla aynı soruna değinir;
ilgili kavram [İlkel birikim](/kavram/ilkel-birikim/) dosyasında ele alınıyor.

## Etki listesi ve çağdaşları

Kaynak, yazarın etkilendiği kişiler arasında Adam Smith ile Max Weber'i de
sayar.[^k1] Etki listesinde geçen yazarlardan biri için tutulan ayrı kayıt, onun
ahlak felsefesinin kapsamlı ve alışılmadık bir sürümünü geliştirdiğini
yazar.[^k2] O yazar [Adam Smith](/dusunur/smith/) dosyasında ele alınıyor.

Aynı yıllarda iktisadi tarih yazan bir başka yazar için tutulan kayıt, onun
kapitalistleri tekelciler olarak gördüğünü yazar.[^k3] O yazar
[Fernand Braudel](/dusunur/braudel/) dosyasında, eserin yazarı ise
[Karl Polanyi](/dusunur/polanyi/) dosyasında ele alınıyor.

## Okumayı sürdürmek için
Bu inceleme eserin metnine bağlanmaz; kullanılan kaynak yazar hakkındaki bir
ansiklopedi maddesidir.[^k1] Eser teliflidir; korpus metninden alıntı yapmaz ve
yalnızca hakkında yazılanı aktarır.
