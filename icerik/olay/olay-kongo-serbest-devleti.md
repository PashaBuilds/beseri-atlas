---
id: olay-kongo-serbest-devleti
tip: olay
baslik: Kongo Serbest Devleti
ozet: >-
...
tarih_bitis: "1908"
bolge: [afrika, avrupa]
eksen: [siyasi, ekonomik, demografik, askeri]
guven_geneli: yaygin
...
kaynaklar:
  - anahtar: k1
    tur: ansiklopedi
    ad: "Wikipedia (EN) - Congo Free State"
  ...
  - anahtar: k2
    ...
    erisim_tarihi: 2026-08-26
    dogrulama_dizesi: "Congo"
    not: "Baska bir devletin konsolosunun hazirladigi resmi rapor"
  - anahtar: k3
    tur: akademik
...
tek bir kişinin kişisel mülkü olarak yönetildi.[^k1] Bu kişi, başka
bir ülkenin kralıydı.
...
Düzenin resmî adında "serbest" ve "devlet" sözcükleri geçer.[^k1] İkisi
de fiilî durumu tarif etmez.
...
Alan, serbest ticarete açık kalacağı gerekçesiyle tanındı.[^k1]
Konferans metinlerinde bu koşul yazılıdır.
...
Gelirin ana kaynağı kauçuktu.[^k2] Toplama işi, köylere kota konarak
yürütüldü.
...
Kotayı uygulayan bir silahlı güç kuruldu.[^k2] Bu güç, yerel olarak
toplanan askerlerden oluşuyordu.

Mermi tüketimi denetleniyordu.[^k1] Harcanan her merminin bir sonuca
karşılık geldiğinin ispatı isteniyordu.

Bu denetim biçimi, kendi sonucunu üretti.[^k2] İspat yükümlülüğü,
raporlarda ayrıntılı biçimde kaydedilmiş bir uygulamaya yol açtı.
...
Bu tür sonuçlar kural yazılırken öngörülmez.[^k2] Kuralın yarattığı
teşvik, kuralın amacından bağımsız işler.

## Kaydın nereden geldiği

Düzene dair en ayrıntılı resmî kayıt, bir başka devletin konsolosu
tarafından hazırlandı.[^k2] Konsolos bölgede dolaştı, tanıkları
dinledi ve raporunu hükümetine sundu.
...
Raporu hazırlayan devlet, kendi sömürge bölgelerinde benzer
uygulamalar yürütüyordu.[^k1] Bu, raporu geçersiz kılmaz.
...
Kauçuk talebi bu dönemde hızla arttı.[^k1] Bisiklet ve otomobil
lastiği, yalıtım ve makine parçaları aynı hammaddeye bağlıydı.

Talep, uzak bir bölgenin ormanını dünya pazarına bağladı.[^k3] Bölgenin
...
Buradaki kauçuk ekilmiyordu, ormandan toplanıyordu.[^k1] Bu, düzeni
plantasyondan ayırır.
...
Ormanda geçen süre, tarlada geçmeyen süredir.[^k1] Kota arttıkça
gıda üretimi düştü.
...
Alanın büyük bölümü imtiyaz şirketlerine devredildi.[^k1] Şirketler
kendi bölgelerinde vergi toplama ve zor kullanma yetkisi aldı.
...
Şirketlerin bir bölümünde yönetim doğrudan pay sahibiydi.[^k1] Yani
denetleyen ile denetlenen aynı çıkarı paylaşıyordu.
...
Rapor yayımlandıktan sonra kurulan kampanya, belirli bir yöntem
kullandı.[^k1] Tanıklıklar, fotoğraflar ve resmî belgeler bir arada
sunuldu.
...
Düzenin savunusu için de bir kampanya yürütüldü.[^k1] Ücretli yayınlar,
karşı raporlar ve yönlendirilmiş geziler kullanıldı.
...
Dönemde yaşanan nüfus kaybının büyüklüğü tartışmalıdır.[^k1] Verilen
tahminler birbirinden kat kat farklıdır.

Farkın iki kaynağı vardır.[^k3] Başlangıç nüfusu bilinmiyor ve
...
Kampanyanın ardından alan, kralın kişisel yönetiminden çıkarıldı.[^k1]
Bir devletin resmî sömürgesi hâline getirildi.
...
Alanın bir kişinin mülkü sayılması, hesap verme yolunu da
kapatıyordu.[^k1] Hiçbir parlamento bütçesini denetlemiyordu.

Kral, kendi ülkesinin hükümetine bu alan hakkında hesap vermek zorunda
...
Düzen, nehir ağının ulaştığı yerlerde etkiliydi.[^k1] Nehirden uzak
bölgelerde denetim zayıf kaldı.
...
Devir sırasında bir bölüm kayıt yok edildi.[^k1] Bu, sonraki
araştırmaların kaynak tabanını doğrudan daralttı.
...
Bölgede yaşayan toplulukların kendi yazılı kaydı bu dönem için çok
sınırlıdır.[^k1] Aktarım büyük ölçüde sözlüdür.
...
Sonraki sömürge dönemi burada izlenmez.[^k3] Kırk yılı aşan o dönem
ayrı bir dosyayı hak eder.

Yerel dilden birincil bir kayıt künyelenememiştir.[^k2] Künyedeki
...
Bölgedeki önceki düzen için
[Kongo Krallığı](/aktor/kongo-kralligi/), emek düzeninin genel biçimi
için [Zorunlu emek düzeni](/kavram/zorunlu-emek-duzeni/) dosyaları
uygundur.[^k3]
