---
id: tartisma-sozlu-gelenek-kaynak-mi
tip: tartisma
...
neden_onemli: >
  ...
  belirliyor. Yazılı kayıt bırakmamış ya da kayıtları yok edilmiş toplumlar,
  sözlü aktarım kaynak sayılmazsa tarih dışında kalır; sayılırsa bu kez
  farklı güvenilirlik ölçütleriyle değerlendirilen iki ayrı tarih doğar.
tarih_baslangic: "1800"
tarih_bitis: "2026"
bolge: [afrika, amerika, kuresel, orta-asya]
eksen: [kulturel, siyasi, felsefi]
guven_geneli: tartismali
etiketler: [sozlu-gelenek, kaynak-elestirisi, tarih-yazimi, arkeoloji, taniklik]
ilgili:
  - kavram-sozlu-tarih
...
hakem_yok: true
pozisyonlar:
  - ad: Sözlü geleneğin kendi eleştiri yöntemi vardır
    savunanlar: [Jan Vansina, Joseph Ki-Zerbo, UNESCO Genel Afrika Tarihi ekibi]
    tez: >
      Sözlü aktarım rastgele değildir; aktarımın kimin tarafından, hangi
      ortamda ve hangi kurallarla yapıldığı bilinirse yazılı kaynaklara
      uygulanan eleştiri yöntemlerinin karşılığı kurulabilir. Kim aktarıyor,
      kime aktarıyor, hangi çıkarı var, hangi kelimeler sabit hangileri
      değişken — bunlar sorulabilir ve cevapları aktarımın güvenilirlik
      derecesini belirler.
    guclu_yani: >
      Yazılı kaynağı bulunmayan bölgelerin geçmişini erişilebilir kılar ve
      bunu keyfî biçimde değil, açıkça belirtilmiş bir yöntemle yapar.
      Ayrıca yazılı kaynakların da aktarım zinciri taşıdığını hatırlatarak
      iki kaynak türü arasındaki farkı derece farkına indirger.
    zayif_yani: >
      Aktarım kurallarının kendisi çoğu zaman aynı sözlü gelenekten
      öğrenilir; yöntem, sınayacağı şeyin verdiği bilgiye dayanır. Ayrıca
      aktarımın ne kadar sabit kaldığı ancak birden çok bağımsız aktarıcı
      karşılaştırılabildiğinde ölçülebilir ve bu her zaman mümkün değildir.
  - ad: Belge önceliği
    savunanlar: [Belge merkezli tarihçilik geleneği, Leopold von Ranke çizgisi]
    tez: >
      Tarih yazımı, olayla eşzamanlı ya da ona yakın tarihli yazılı belgeye
      dayanmalıdır. Belge, yazıldığı anda sabitlenir ve sonradan
      değiştirilmesi iz bırakır; sözlü aktarımda böyle bir sabitleme yoktur.
      Sabitlenmemiş bir aktarımdan olgu çıkarmak, doğrulanamaz bir iddiayı
      olgu olarak kaydetmek olur.
    guclu_yani: >
      Doğrulama ölçütü açık ve uygulanabilirdir; iki araştırmacı aynı belgeye
      bakıp aynı metni görür. Kaydın ne zaman oluştuğu bilinebildiği için
      sonradan eklenmiş katmanlar ayrıştırılabilir.
    zayif_yani: >
      Ölçüt uygulandığında dünyanın büyük bölümü tarih dışında kalır ve bu
      dışarıda kalış rastgele değildir: belge bırakmamış ya da belgeleri yok
      edilmiş toplumlar sistematik olarak sessizleşir. Ayrıca belgelerin de
      taraflı olduğu, kimin ne için yazdığına bağlı olduğu göz ardı edilir.
  - ad: Arkeolojik doğrulama şartı
    savunanlar: [Süreçsel arkeoloji çizgisi, maddi kanıt önceliği yaklaşımı]
    tez: >
      Sözlü aktarım, ancak kazı bulguları, yapı kalıntıları ya da tarihlenebilir
      malzeme tarafından desteklendiği ölçüde kaynak sayılabilir. Anlatının
      kendisi bir hipotez üretir; hipotezi sınayacak olan maddi kayıttır.
    guclu_yani: >
      İki bağımsız kanıt türünün birbirini denetlemesini sağlar ve
      doğrulanmış ile doğrulanmamış aktarımı ayırt edilebilir kılar. Uygulandığı
      yerlerde somut sonuç vermiştir.
    zayif_yani: >
      Maddi kayıt bırakmayan olayları — kararlar, anlaşmalar, soy ilişkileri,
      inanç değişimleri — kapsam dışında bırakır. Ayrıca kazı yapılabilmesi
      kaynak, izin ve altyapı gerektirir; şart, zengin bölgelerin geçmişini
      yoksul bölgelerinkinden daha doğrulanabilir kılar.
  - ad: Sözlü gelenek kaynak değil konudur
    savunanlar: [Folklor ve edebiyat incelemesi çizgisi, anlatı çözümlemesi yaklaşımı]
    tez: >
      Sözlü anlatılar geçmişteki olaylar hakkında değil, onları anlatan
      topluluğun bugünkü kendini anlama biçimi hakkında bilgi verir.
      Bir soy anlatısı, soyun gerçekliğini değil o soy iddiasının bugün ne işe
      yaradığını gösterir ve incelenmesi gereken budur.
    guclu_yani: >
      Anlatıdan olgu çıkarmanın doğurduğu doğrulama sorununu tümüyle
      ortadan kaldırır ve anlatıyı kendi başına değerli bir inceleme nesnesi
      yapar. Anlatının neden o biçimde kurulduğu sorusu, olgu sorusundan
      bağımsız olarak cevaplanabilir.
    zayif_yani: >
      Uygulandığında, yazılı kaynağı bulunmayan toplumların olay tarihi
      yazılamaz hâle gelir; bu toplumlar hakkında yalnızca kültürel çözümleme
      yapılabilir. Sonuç, belge önceliği konumuyla aynı yere varır ama farklı
      bir gerekçeyle.
kaynaklar:
  - anahtar: k1
    tur: akademik
